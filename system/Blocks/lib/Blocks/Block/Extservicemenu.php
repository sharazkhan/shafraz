 ...
 */
class Blocks_Block_Extservicemenu extends Zikula_Controller_AbstractBlock {

    /**
     * initialise block
     */
    public function init() {
        SecurityUtil::registerPermissionSchema('ExtendedServiceMenublock::', 'Block ID:Link ID:');
    }
    ...
    public function info() {
        return array('module' => $this->name,
            'text_type' => $this->__('Extended Service Menu'),
            'text_type_long' => $this->__('Extended service menu block'),
            'allow_multiple' => true,
            'form_content' => false,
            'form_refresh' => false,
            'show_preview' => true,
    ...
    public function display($blockinfo) {
        ...
        //echo "hellooo";
        // security check
        if (!SecurityUtil::checkPermission('ExtendedServiceMenublock::', $blockinfo['bid'] . '::', ACCESS_COMMENT)) {
            return;
        }
        ...
        if (empty($vars['template'])) {
            $vars['template'] = 'blocks_block_extservicemenu.tpl';
        }
        // stylesheet to use
        ...
        if (!empty($vars['links'][$thislang])) {
            ...
            foreach ($vars['links'][$thislang] as $linkid => $link) {
                ...
                $denied = !SecurityUtil::checkPermission('ExtendedMenublockZselex::', $blockinfo['bid'] . ':' . $linkid . ':', ACCESS_READ);
        ...
        if (!empty($vars['displaymodules'])) {
            ...
            if (count($menuitems) > 0) {
                ...
                if (SecurityUtil::checkPermission('ExtendedServiceMenublock::', $blockinfo['bid'] . '::', ACCESS_ADMIN)) {
        ...
        $this->view->assign('menuitems', $menuitems)
                ...
                ->assign('access_edit', Securityutil::checkPermission('ExtendedServiceMenublock::', $blockinfo['bid'] . '::', ACCESS_EDIT));
    ...
    public function modify($blockinfo) {
        ...
        // return the output
        return $this->view->fetch('blocks_block_extservicemenu_modify.tpl');
    }
