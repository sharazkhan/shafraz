...
class News_Api_User extends Zikula_AbstractApi {
    ...
    public function getArticleInfo($info) {
        ...
        if (ModUtil::available('EZComments') && $info['allowcomments'] == 1) {
            $info['commentcount'] = ModUtil::apiFunc('EZComments', 'user', 'countitems', array('mod' => 'News',
                        'objectid' => $info['sid'],
                        'status' => 0));
        }
    ...
    public function decodeurl($args) {
        ...
        if ($func == 'display' || $func == 'displaypdf') {
           // echo "comes here"; exit;
            ...
            //echo "<pre>"; print_r($permalinkkeys); echo "</pre>";
            // get rid of unused vars
            $args['vars'] = array_slice($args['vars'], $nextvar);

            // echo "<pre>"; print_r($args['vars']); echo "</pre>";
    ...
    protected function generateWhere($args) {
        ...
        /*
          ...
          if (!SecurityUtil::checkPermission('News::', '::', ACCESS_ADMIN) && $_REQUEST['func'] == 'view') {   // user shop articles
          //echo "comes here";
          ...
          array_push($queryargs, "sid IN(SELECT news_id FROM zselex_shop_news  WHERE shop_id='" . $shop_id . "')");
          }


          if (!SecurityUtil::checkPermission('News::', '::', ACCESS_ADMIN) && $_REQUEST['type'] == 'admin' && $_REQUEST['func'] == 'view' || $_REQUEST['type'] == 'user' && $_REQUEST['func'] == 'view') {   // user shop articles
          // echo "Comes Here";
          //array_push($queryargs, "cr_uid='" . UserUtil::getVar('uid') . "'");
          }
        ...
        if ($_REQUEST['type'] != 'admin') {
            array_push($queryargs, "sid NOT IN(SELECT news_id FROM  zselex_shop_news )");
        }
