 ...
 {if $pictures gt 0}
   ...
   {if $readperm}
       ...
       <a href="{modurl modname='ZSELEX' type='user' func='display' shop_id=$smarty.request.shop_id sid=$sid}"> 
    {/if}

   <img src="{$baseurl}{$picupload_uploaddir}/pic_sid{$sid}-0-thumb.jpg">
...
<p class="sec1L">
    ...
    {elseif $smarty.request.module eq 'ZSELEX' AND $smarty.request.func eq 'display'}
      <a href="{modurl modname='ZSELEX' type='user' func='display' shop_id=$smarty.request.shop_id sid=$sid}">{gt text='Read this full article'}...<img src="{$imagepath}/Arrow1.png" /></a>
    {else}  
      <a href="{pnmodurl modname='News' func='display' sid=$sid}">{gt text='Read this full article'}...<img src="{$imagepath}/Arrow1.png" /></a>
    {/if}
    </p>
