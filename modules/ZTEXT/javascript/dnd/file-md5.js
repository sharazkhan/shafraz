/**
 * File Md5 from Alban Xhaferllari
 * Based on: 
 * Crypto-JS v2.5.3
...
(function(){
	...
	function readBlob(b) 
	{
	    ...
	    for (var d = g.size, e = 0, f = 1048576 > d ? d : 1048576, h; e < d;) 
	    	h = g.slice(e, f), h = a.readAsArrayBuffer(h), 
	    	updateMd5(h, e, f, d),
	    	postMessage({status: "progress", progress: 100 * (f / d) }), 
	    	e = f, f += 1048576, f > d && (f = d);
	    g = {};
	    g.md5 = Crypto.util.bytesToHex(Crypto.util.wordsToBytes(Crypto.util.endian(self.md5hash)));
	    
	    
	    postMessage({status: "end", result: g.md5 });
	}
	
	
	//AS WEBWOKER JOB
	self.addEventListener("message", function (b) { readBlob(b.data); }, false);
})();