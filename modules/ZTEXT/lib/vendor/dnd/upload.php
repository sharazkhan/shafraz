...
class RealAjaxUploader {
    ...
    function __construct($deny_ext = array()) {
        ...
        if (isset($_REQUEST['ax-allow-ext']))
            $this->setAllowExt(!empty($_REQUEST['ax-allow-ext']) ? explode('|', $_REQUEST['ax-allow-ext']) : array() );
        ...
        $this->file_orig_name = $this->file_name;
        $_REQUEST['file_orig_name'] = $this->file_orig_name;
        ...
        $this->get_data = serialize($_REQUEST);
    ...
    private function checkFileExits($upload_path = '') {
        ...
        if (!$this->override) {
            ...
            /*
              $file_data = pathinfo($this->file_name);
              $file_base = $file_data['filename'];
              $file_ext = $file_data['extension'];
              $filename = $file_base . "-" . time() . "." . $file_ext;
              $this->file_name = $filename;
    ...
    private function uploadAjax() {
        ...
        if ($isLast != 'true') {
            ...
            $this->file_name = $file_parts['filename'] . "-" . $unique_id . "." . $file_parts['extension'];
    ...
    private function finish() {
         ...
         */
        //  $finish_func_arg = array('params' => $this->get_data);
        $finish_func_arg = array($this->upload_path . '/', $this->file_name, $this->get_data);
        try {
            call_user_func_array(array('ZTEXT_Api_DndCallBack', $this->finish_function), $finish_func_arg);
        } catch (Exception $e) {
            echo $e->getTraceAsString();
        }
...
if (isset($EMAIL_TO) && $EMAIL_TO)
    $uploader->setEmail($EMAIL_TO, $EMAIL_FROM); //the email will be send after file upload
