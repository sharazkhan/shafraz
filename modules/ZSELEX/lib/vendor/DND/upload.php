...
/*
 * function product_success($file_path) {
 * mysql_connect('localhost', 'root', '');
 * mysql_select_db('test');
 * $sql = "INSERT INTO content(content)VALUES('hellooooooo')";
 * mysql_query($sql);
 * }
 */
...
class RealAjaxUploader
{
    ...
    function __construct($deny_ext = array())
    {
        ...
        if (isset($_REQUEST ['ax-allow-ext']))
                $this->setAllowExt(!empty($_REQUEST ['ax-allow-ext']) ? explode('|',
                        $_REQUEST ['ax-allow-ext']) : array() );
        if (isset($_REQUEST ['ax-override'])) $this->setOverride(true);
        // set deny
        $this->deny_ext          = $deny_ext;
        $this->file_check_folder = $_REQUEST ['file_check_folder'];

        // active parameters neccessary for upload
        $this->file_name = isset($_REQUEST ['ax-file-name']) ? $_REQUEST ['ax-file-name']
                : $_FILES ['ax_file_input'] ['name'];

        $path_parts           = pathinfo($this->file_name);
        $files_name           = $path_parts ['filename'];
        $this->file_orig_name = $files_name;
        ...
        // $fileName = preg_replace("/[^A-Za-z0-9_-]/", "", $files_name . '_' . time()) . ".$extension";
        $this->file_name = $fileName;
        $this->file_size = isset($_REQUEST ['ax-file-size']) ? $_REQUEST ['ax-file-size']
                : $_FILES ['ax_file_input'] ['size'];
        ...
        $this->product_cat_id  = $_REQUEST ['product_cat_id'];
        $this->manufacturer_id = $_REQUEST ['manufacturer_id'];
    }
    ...
    public function customError($error, $mgs)
    {
        if ($error) {
    ...
    private function createThumbGD($quality = 75)
    {
        ...
        $maxheight  = isset($_REQUEST ['ax-thumbHeight']) ? $_REQUEST ['ax-thumbHeight']
                : 0;
        $maxwidth   = isset($_REQUEST ['ax-thumbWidth']) ? $_REQUEST ['ax-thumbWidth']
                : 0;
        $postfix    = isset($_REQUEST ['ax-thumbPostfix']) ? $_REQUEST ['ax-thumbPostfix']
                : '_thumb';
        $thumb_path = isset($_REQUEST ['ax-thumbPath']) ? $_REQUEST ['ax-thumbPath']
                : '';
        $format     = isset($_REQUEST ['ax-thumbFormat']) ? $_REQUEST ['ax-thumbFormat']
                : 'png';
        ...
        $thumb_path .= (!in_array(substr($thumb_path, - 1),
                array(
                '\\',
                '/'
        ...
        if ($width_orig > 0 && $height_orig > 0) {
            ...
            imagecopyresampled($thumb, $image, 0, 0, 0, 0, $newW, $newH,
    ...
    private function checkSize()
    {
        ...
        $max_size      = !is_numeric($rang) && !is_numeric($max_file_size) ? str_replace($rang,
                '', $max_file_size) : $max_file_size;
        if ($rang && $max_size) {
            switch (strtoupper($rang)) { // 1024 or 1000??
                case 'Y' :
                    $max_size = $max_size * 1024; // Yotta byte, will arrive such day???
                case 'Z' :
    ...
    private function checkFileExits($upload_path = '')
    {
        ...
        if (isset($this->file_check_folder) && !empty($this->file_check_folder)) {
            $upload_path = $upload_path.$this->file_check_folder.'/';
        }
    ...
    private function checkExt()
    {
        ...
        if (in_array($file_ext, $this->deny_ext) || (!in_array($file_ext,
                $this->allow_ext) && count($this->allow_ext))) {
            return false;
        }
        return true;
    }
    ...
    {
        // exit;
        // $this->message(-1, 'Diskquota exceeded');
        $currByte      = isset($_REQUEST ['ax-start-byte']) ? $_REQUEST ['ax-start-byte']
                : 0;
        $isLast        = isset($_REQUEST ['ax-last-chunk']) ? $_REQUEST ['ax-last-chunk']
                : 'true';
        $this->shop_id = $_REQUEST ['shop_id'];
        ...
        $post_bytes = file_get_contents(isset($_FILES ['ax_file_input']) ? $_FILES ['ax_file_input'] ['tmp_name']
                    : 'php://input' );
        ...
        $try = 20;
        while (@file_put_contents($full_path, $post_bytes, $flag) === false && $try
        > 0) {
            usleep(50);
            $try --;
        }
        ...
        if ($isLast != 'true') {
            ...
            // $this->file_name = $file_parts['filename'] . "-" . time() . "." . $file_parts['extension'];
            $unique_id       = md5(uniqid(time(), true));
            $this->file_name = $file_parts ['filename']."_".$unique_id.".".$file_parts ['extension'];
            $this->checkFileExits($this->upload_path);
            $ret             = rename($full_path,
                $this->upload_path.$this->file_name); // move file from temp dir to upload dir TODO this can be slow on big files and diffrent drivers
            if ($ret) {
    ...
    private function uploadStandard()
    {
        ...
        $result    = move_uploaded_file($_FILES ['ax_file_input'] ['tmp_name'],
            $full_path); // make the upload
    ...
    public function uploadFile1($shop_id, $file_size, $func = '')
    {
        // $this->message(-1, $this->shop_id);
        // $this->message(-1, "shopid :" . $shop_id . " " . "file size :" . $file_size . " " . "func :" . $func);
        if (!empty($func)) {
            $diskquotaCheck = ModUtil::apiFunc('ZSELEX', 'Service', $func,
                    $args           = array(
                    'shop_id' => $shop_id,
                    'filesize' => $file_size
                ));
            if ($diskquotaCheck ['error']) {
                $this->message(- 1, $diskquotaCheck ['msg']);
            }
        }
        if ($this->checkFile()) { // this checks every chunk FIXME is right?
            $is_ajax = isset($_REQUEST ['ax-last-chunk']) && isset($_REQUEST ['ax-start-byte']);
    ...
    private function finish()
    {
        ...
        // call_user_func_array(array('ZSELEX_Controller_DndCallback', $this->finish_function), array($this->upload_path . '/', $this->file_name, $shop_id . '-' . $this->file_name)); // testing
        // call_user_func_array(array('ZSELEX_Controller_DndCallBack', $this->finish_function), array($this->upload_path . '/', $this->file_name, $shop_id));
        $finish_func_arg = array(
            $this->upload_path.'/',
            $this->file_name,
            $this->file_orig_name,
            $shop_id
        );
        if (isset($this->product_cat_id)) {
            $finish_func_arg ['product_cat_id'] = $this->product_cat_id;
        }
        if (isset($this->manufacturer_id)) {
            $finish_func_arg ['manufacturer_id'] = $this->manufacturer_id;
        }
        // call_user_func_array(array('ZSELEX_Api_DndCallBack', $this->finish_function), array($this->upload_path . '/', $this->file_name, $this->file_orig_name, $shop_id));
        call_user_func_array(array(
            'ZSELEX_Api_DndCallBack',
            $this->finish_function
            ), $finish_func_arg);
        if ($this->finish_function && function_exists($this->finish_function)) {
            try {
                // call_user_func($this->finish_function, $this->upload_path . $this->file_name); original
    ...
    private function checkFile()
    {
        ...
        if (isset($_FILES ['ax_file_input'])) {
            if ($_FILES ['ax_file_input'] ['error'] !== UPLOAD_ERR_OK) {
                $this->message(- 1,
                    $this->upload_errors [$_FILES ['ax_file_input'] ['error']]);
            }
        }
        ...
        if (!$this->checkSize()) {
            $this->message(- 1,
                'File size exceeded maximum allowed: '.$this->max_file_size);
        }
        return true;
    }
