 ...
 */
class ZSELEX_Api_Userusers extends Zikula_AbstractApi {
	...
	public function getAll($args) {
		...
		$where [] = "({$usersColumn['activated']} NOT IN (" . implode ( ', ', array (
				Users_Constant::ACTIVATED_PENDING_REG,
				Users_Constant::ACTIVATED_PENDING_DELETE 
		) ) . '))';
		
		$shop_id = $_REQUEST ['shop_id'];
		
		// if (!SecurityUtil::checkPermission('ZSELEX::', '::', ACCESS_ADMIN)) {
		// $where[] = "uid IN (SELECT parentId FROM zselex_parent WHERE childType='SHOP' AND parentType='SHOPADMIN' AND cr_uid='" . UserUtil::getVar('uid') . "')";
		// }
		// shop users
		if ($userType == 'shopadmin') {
			// $where[] = "uid IN (SELECT parentId FROM zselex_parent WHERE childType='SHOP' AND parentType='SHOPADMIN' AND childId='" . $shop_id . "')";
			$where [] = "uid IN (SELECT user_id FROM zselex_shop_admins WHERE shop_id='" . $shop_id . "')";
			// $where[] = "uid IN (SELECT parentId FROM zselex_parent WHERE childType='SHOP' AND parentType='SHOPADMIN' AND childId='".$shop_id."' AND cr_uid='" . UserUtil::getVar('uid') . "')";
		} elseif ($userType == 'shopowner') {
			// $where[] = "uid IN (SELECT user_id FROM zselex_shop WHERE shop_id='" . $shop_id . "')";
			
			$where [] = "uid IN (SELECT user_id FROM zselex_shop_owners WHERE shop_id='" . $shop_id . "')";
		}
		
		// OR uid IN(SELECT user_id FROM zselex_shop WHERE shop_id='".$shop_id."')
		...
		if (isset ( $subject ) && ! empty ( $subject )) {
			...
			switch ($notificationType) {
				...
				case 'welcome' :
					...
					break;
				default :
					$mailerArgs ['subject'] = $this->__f ( 'A message from %s.', array (
							System::getVar ( 'sitename', System::getBaseUrl () ) 
					) );
			}
		}
	...
	public function mailUname($args) {
		...
		if (! isset ( $args ['id'] ) || empty ( $args ['id'] ) || ! isset ( $args ['idfield'] ) || empty ( $args ['idfield'] ) || (($args ['idfield'] != 'email') && ($args ['idfield'] != 'uid'))) {
			$this->registerError ( LogUtil::getErrorMsgArgs () );
			return false;
		}
	...
	public function mailConfirmationCode($args) {
		...
		if (! isset ( $args ['id'] ) || empty ( $args ['id'] ) || ! isset ( $args ['idfield'] ) || empty ( $args ['idfield'] ) || (($args ['idfield'] != 'uname') && ($args ['idfield'] != 'email') && ($args ['idfield'] != 'uid'))) {
			$this->registerError ( LogUtil::getErrorMsgArgs () );
			return false;
		}
		...
		if ($user) {
			...
			if ($confirmationCodeHash !== false) {
				...
				DBUtil::deleteWhere ( 'users_verifychg', "({$verifychgColumn['uid']} = {$user['uid']}) AND ({$verifychgColumn['changetype']} = " . Users_Constant::VERIFYCHGTYPE_PWD . ")" );
	...
	public function checkConfirmationCode($args) {
		...
		if (! isset ( $args ['id'] ) || empty ( $args ['id'] ) || ! isset ( $args ['idfield'] ) || empty ( $args ['idfield'] ) || ! isset ( $args ['code'] ) || empty ( $args ['code'] ) || (($args ['idfield'] != 'uname') && ($args ['idfield'] != 'email'))) {
			$this->registerError ( LogUtil::getErrorMsgArgs () );
			return false;
		}
		...
		if (! $user) {
			...
			$tables = DBUtil::getTables ();
			$verifychgColumn = $tables ['users_verifychg_column'];
			$verifychgObj = DBUtil::selectObject ( 'users_verifychg', "({$verifychgColumn['uid']} = {$user['uid']}) AND ({$verifychgColumn['changetype']} = " . Users_Constant::VERIFYCHGTYPE_PWD . ")" );
	...
	public function accountLinks() {
		...
		foreach ( $mods as $mod ) {
			...
			if ($mod ['type'] == 3 && ! in_array ( $mod ['name'], array (
					'Admin',
					'Categories',
					'Groups',
					'Theme',
	...
	public function savePreEmail($args) {
		...
		DBUtil::deleteWhere ( 'users_verifychg', "({$verifychgColumn['uid']} = {$uid}) AND ({$verifychgColumn['changetype']} = " . Users_Constant::VERIFYCHGTYPE_EMAIL . ")" );
		...
		$viewArgs = array (
				...
				'sitename' => System::getVar ( 'sitename' ),
				'url' => ModUtil::url ( $this->name, 'user', 'confirmChEmail', array (
	...
	public function getUserPreEmail() {
		...
		$item = DBUtil::selectObject ( 'users_verifychg', "({$verifychgColumn['uid']} = {$uid}) AND ({$verifychgColumn['changetype']} = " . Users_Constant::VERIFYCHGTYPE_EMAIL . ")" );
	...
	public function processRegistrationErrorsForDisplay($args) {
		...
		if (isset ( $args ['registrationErrors'] ) && is_array ( $args ['registrationErrors'] ) && ! empty ( $args ['registrationErrors'] )) {
			...
			foreach ( $registrationErrors as $field => $messageList ) {
				...
				$errorMessages = array_merge ( $errorMessages, is_array ( $messageList ) ? $messageList : array (
						$messageList 
				) );
			}
		}
