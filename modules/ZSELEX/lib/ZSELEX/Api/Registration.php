 ...
 */
class ZSELEX_Api_Registration extends Zikula_AbstractApi {
	/**
	...
	private function currentUserIsAdmin() {
		...
		return UserUtil::isLoggedIn () && SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
	}
	...
	public function getPasswordErrors($args) {
		...
		if ($reginfo ['pass'] != Users_Constant::PWD_NO_USERS_AUTHENTICATION) {
			if (! isset ( $reginfo ['pass'] ) || empty ( $reginfo ['pass'] )) {
				...
				$passwordErrors ['pass'] = $this->_fn ( 'Your password must be at least %s character long', 'Your password must be at least %s characters long', $minPasswordLength, $minPasswordLength );
			...
			if (! $this->currentUserIsAdminOrSubAdmin ()) {
				if (! isset ( $reginfo ['passreminder'] ) || empty ( $reginfo ['passreminder'] )) {
					...
					if (! empty ( $testPass ) && (strlen ( $testPassreminder ) >= strlen ( $testPass )) && (stristr ( $testPassreminder, $testPass ) !== false)) {
						...
						if (! empty ( $testPass ) && ! empty ( $testPassreminder ) && (strlen ( $testPass ) >= $minPasswordLength) && (strlen ( $testPassreminder ) >= strlen ( $testPass )) && (stristr ( $testPassreminder, $testPass ) !== false)) {
	...
	public function getRegistrationErrors($args) {
		...
		if (! isset ( $reginfo ['uname'] ) || empty ( $reginfo ['uname'] )) {
			...
			$registrationErrors ['uname'] = $this->__f ( 'The user name you entered is too long. The maximum length is %1$d characters.', array (
					Users_Constant::UNAME_VALIDATION_MAX_LENGTH 
			) );
		} else {
			$tempValid = true;
			if (! $isAdmin) {
		...
		if (isset ( $reginfo ['theme'] ) && ! empty ( $reginfo ['theme'] )) {
			$theme_id = ThemeUtil::getIDFromName ( $reginfo ['theme'] );
			if (! $theme_id) {
				$registrationErrors ['theme'] = $this->__f ( '\'%1$s\' is not a valid theme.', array (
						$reginfo ['theme'] 
				) );
			}
		}
	...
	public function registerNewUser($args) {
		...
		$adminWantsVerification = $isAdminOrSubAdmin && ((isset ( $args ['usermustverify'] ) ? ( bool ) $args ['usermustverify'] : false) || ! isset ( $reginfo ['pass'] ) || empty ( $reginfo ['pass'] ));
	 ...
	 *
	 * @param
	 *        	array &$obj The array appropriate for the $table; passed by reference (this function will cause
	 ...
	 *
	 * @param array $reginfo
	 *        	Contains the data gathered about the user for the registration record.
	 * @param bool $userNotification
	 *        	Whether the user should be notified of the new registration or not; however
	 *        	if the user's password was created for him, then he will receive at
	 *        	least that notification without regard to this setting.
	 * @param bool $adminNotification
	 *        	Whether the configured administrator notification e-mail address should be
	 *        	sent notification of the new registration.
	 * @param string $passwordCreatedForUser
	 *        	The password that was created for the user either automatically or by an
		...
		if ($userObj) {
			...
			if ($adminNotification || $userNotification || ! empty ( $passwordCreatedForUser )) {
				...
				if (! $reginfo ['isverified'] && (($approvalOrder != Users_Constant::APPROVAL_BEFORE) || $reginfo ['isapproved'])) {
					...
					$notificationSent = ModUtil::apiFunc ( $this->name, 'user', 'sendNotification', array (
				...
				if ($adminNotification) {
					...
					if (! empty ( $notificationEmail )) {
						$notificationSent = ModUtil::apiFunc ( $this->name, 'userusers', 'sendNotification', array (
	 ...
	 *        	Contains the data gathered about the user for the registration record.
	 * @param bool $userNotification
	 *        	Whether the user should be notified of the new registration or not;
	 ...
	 * @param bool $adminNotification
	 *        	Whether the configured administrator notification e-mail address should
	 *        	be sent notification of the new registration.
	 * @param string $passwordCreatedForUser
	 *        	The password that was created for the user either automatically or by
	...
	protected function createUser(array $reginfo, $userNotification = true, $adminNotification = true, $passwordCreatedForUser = '') {
		...
		if ($userObj) {
			...
			$groupAdded = ModUtil::apiFunc ( 'Groups', 'user', 'adduser', array (
					'gid' => $defaultGroup,
					'uid' => $userObj ['uid'] 
			) );
			if (! $groupAdded) {
	 ...
	 *
	 * @param array $args
	 *        	All parameters passed to this function; either id, uname, or email must be specified, but
	...
	public function get($args) {
		if ((! UserUtil::isLoggedIn () && ! SecurityUtil::checkPermission ( 'Users::', '::', ACCESS_READ )) || (UserUtil::isLoggedIn () && ! SecurityUtil::checkPermission ( 'Users::', '::', ACCESS_MODERATE ))) {
			throw new Zikula_Exception_Forbidden ();
		}
		...
		if ((! isset ( $args ['uid'] ) && ! isset ( $args ['uname'] ) && ! isset ( $args ['email'] )) || (isset ( $args ['uid'] ) && (isset ( $args ['uname'] ) || isset ( $args ['email'] ))) || (isset ( $args ['uname'] ) && isset ( $args ['email'] )) || (isset ( $args ['email'] ) && ! $uniqueEmails)) {
			$this->registerError ( LogUtil::getErrorMsgArgs () );
			return false;
		}
	 ...
	 *
	 * @param array $filter
	 *        	The filter, see getAll() and countAll().
	 *        	
			...
			if (preg_match ( '/^IS (?:NOT )?NULL/i', $value ['operator'] )) {
				...
				if (is_null ( $value ['operand'] ) || (is_array ( $value ['operand'] ) && empty ( $value ['operand'] ))) {
					...
					foreach ( $value ['operand'] as $key => $operandItem ) {
						$value ['operand'] [$key] = preg_replace ( array (
								'/\\\'/',
								'/\\\\/' 
						), array (
								'\\\'',
					...
					$where [] = $regColumn [$field] . ' ' . strtoupper ( $value ['operator'] ) . " ('" . implode ( "', '", (is_array ( $value ['operand'] ? $value ['operand'] : array (
							$value ['operand'] 
					) )) ) . "')";
				}
			} else {
	...
	public function getAll($args) {
		if ((! UserUtil::isLoggedIn () && ! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN )) || (UserUtil::isLoggedIn () && ! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN ))) {
			throw new Zikula_Exception_Forbidden ();
		}
		
		if (isset ( $args ['limitoffset'] ) && is_numeric ( $args ['limitoffset'] ) && (( int ) $args ['limitoffset'] == $args ['limitoffset']) && ($args ['limitoffset'] > 0)) {
			$limitOffset = $args ['limitoffset'];
		} else {
			$limitOffset = - 1;
		}
		
		if (isset ( $args ['limitnumrows'] ) && is_numeric ( $args ['limitnumrows'] ) && (( int ) $args ['limitnumrows'] == $args ['limitnumrows']) && ($args ['limitnumrows'] > 0)) {
		...
		if (isset ( $args ['filter'] )) {
			...
			$where = $this->whereFromFilter ( array (
					'activated' => Users_Constant::ACTIVATED_PENDING_REG 
			) );
		}
		if ($where === false) {
			return false;
	...
	public function countAll($args) {
		...
		if (isset ( $isVerifiedFilter )) {
			...
			$users = DBUtil::selectObjectArray ( 'users', $where, null, null, null, null, null, null, array (
					'uid' 
			) );
			$count = 0;
			if ($users) {
	...
	public function remove($args) {
		...
		if (isset ( $args ['uid'] )) {
			...
			$uid = $args ['uid'];
		} elseif (! isset ( $args ['reginfo'] ) || empty ( $args ['reginfo'] ) || ! is_array ( $args ['reginfo'] ) || ! isset ( $args ['reginfo'] ['uid'] ) || empty ( $args ['reginfo'] ['uid'] ) || ! is_numeric ( $args ['reginfo'] ['uid'] )) {
			$this->registerError ( LogUtil::getErrorMsgArgs () );
			return false;
		} else {
	...
	protected function purgeExpired() {
		...
		if ($regExpireDays > 0) {
			...
			$where = "WHERE ({$verifyChgColumn['changetype']} = " . Users_Constant::VERIFYCHGTYPE_REGEMAIL . ") " . "AND ({$verifyChgColumn['created_dt']} IS NOT NULL) " . "AND ({$verifyChgColumn['created_dt']} != '0000-00-00 00:00:00') " . "AND ({$verifyChgColumn['created_dt']} < '{$staleRecordUTCStr}')";
	...
	public function getVerificationCode($args) {
		...
		$dbinfo = DBUtil::getTables ();
		$verifyChgColumn = $dbinfo ['users_verifychg_column'];
		$where = "WHERE ({$verifyChgColumn['uid']} = {$args['uid']}) AND ({$verifyChgColumn['changetype']} = " . Users_Constant::VERIFYCHGTYPE_REGEMAIL . ")";
	...
	public function approve($args) {
		...
		if (isset ( $args ['force'] ) && $args ['force']) {
			if (! isset ( $reginfo ['email'] ) || empty ( $reginfo ['email'] )) {
				$this->registerError ( $this->__f ( 'Error: Unable to force registration for \'%1$s\' to be verified during approval. No e-mail address.', array (
						$reginfo ['uname'] 
				) ) );
				return false;
			}
