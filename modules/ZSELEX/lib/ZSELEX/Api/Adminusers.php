 ...
 */
class ZSELEX_Api_Adminusers extends Zikula_AbstractApi {
	/**
	 * Find users.
	 *
				...
				if ($value) {
					switch ($arg) {
						...
						case 'ugroup' :
							...
							break;
						case 'regdateafter' :
							$where [] = "({$userscolumn['user_regdate']} > '" . DataUtil::formatForStore ( $value ) . "')";
							break;
						case 'regdatebefore' :
							$where [] = "({$userscolumn['user_regdate']} < '" . DataUtil::formatForStore ( $value ) . "')";
							break;
	...
	public function sendmail($args) {
		...
		if (! empty ( $missingFields )) {
			...
			$msg = _fn ( 'Error! The required field \'%2$s\' was blank or missing', 'Error! %1$d required fields were blank or missing: \'%2$s\'.', $count, array (
					$count,
		...
		foreach ( $sendmail ['recipientsemail'] as $uid => $recipient ) {
			if (in_array ( $uid, $recipientUidList )) {
				$bcclist [] = array (
						'name' => $sendmail ['recipientsname'] [$uid],
						'address' => $recipient 
				);
				$recipientlist [] = $recipient;
			}
			if (count ( $bcclist ) == $sendmail ['batchsize']) {
				if (ModUtil::apiFunc ( 'Mailer', 'user', 'sendmessage', array (
		...
		if ($recipientscount > 0) {
			$this->registerStatus ( $this->_fn ( 'Done! E-mail message has been sent to %1$d user. ', 'Done! E-mail message has been sent to %1$d users. ', $recipientscount, array (
					$recipientscount 
			) ) );
		}
	...
	public function getLinks() {
		...
		if (SecurityUtil::checkPermission ( "{$this->name}::", '::', ACCESS_MODERATE )) {
			...
			if ($pending) {
				$links [] = array (
						'url' => ModUtil::url ( $this->name, 'admin', 'viewRegistrations' ),
		...
		if (SecurityUtil::checkPermission ( "{$this->name}::", '::', $createUserAccessLevel )) {
			$submenulinks = array ();
			$submenulinks [] = array (
					'url' => ModUtil::url ( $this->name, 'admin', 'newUser' ),
			...
			if (SecurityUtil::checkPermission ( "{$this->name}::", '::', ACCESS_ADMIN )) {
				$submenulinks [] = array (
						'url' => ModUtil::url ( $this->name, 'admin', 'exporter' ),
		...
		if (SecurityUtil::checkPermission ( "{$this->name}::", '::', ACCESS_MODERATE )) {
			$links [] = array (
					'url' => ModUtil::url ( $this->name, 'admin', 'search' ),
		...
		if (SecurityUtil::checkPermission ( 'Users::MailUsers', '::', ACCESS_MODERATE )) {
			$links [] = array (
					'url' => ModUtil::url ( $this->name, 'admin', 'mailUsers' ),
	...
	public function createImport($args) {
		...
		if (! $usersInDB) {
			$this->registerError ( $this->__ ( 'Error! The users have been created but something has failed trying to get them from the database. ' . 'Now all these users do not have group.' ) );
		...
		if ($modinfo ['state'] == ModUtil::TYPE_SYSTEM) {
			...
			foreach ( $importValues as $value ) {
				...
				if ($value ['activated'] && $value ['sendmail']) {
					...
					$subject = $this->__f ( 'Password for %1$s from %2$s', array (
							$value ['uname'],
							$sitename 
					) );
					$sendMessageArgs = array (
