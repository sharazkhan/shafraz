 ...
 */
class ZSELEX_Controller_Adminusers extends Zikula_AbstractController {
	...
	public function view() {
		...
		if (! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN )) {
			throw new Zikula_Exception_Forbidden ();
		}
		...
		if (ZSELEX_Controller_Base_Admin::shopPermission ( $shop_id ) < 1) {
			return LogUtil::registerPermissionError ();
		}
		
		//ZSELEX_Controller_Base_Admin::payMentAlert ( $shop_id );
		...
		// $userList = ModUtil::apiFunc('ZSELEX', 'userusers', 'getAll', $getAllArgs);
		$userList = $repo->getShopAdmins ( array (
				'shop_id' => $shop_id 
		) );
		...
		foreach ( $groups as $group ) {
			$userGroupsAccess [$group ['gid']] = array (
					'gid' => $group ['gid'] 
			);
			
			// rewrite the groups array with the group id as key and the group name as value
			$groupsArray [$group ['gid']] = array (
					'name' => DataUtil::formatForDisplayHTML ( $group ['name'] ) 
			);
		}
		...
		$currentUserHasReadAccess = SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
		$currentUserHasModerateAccess = SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
		$currentUserHasEditAccess = SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
		$currentUserHasDeleteAccess = SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
		...
		foreach ( $userList as $key => $userObj ) {
			...
			$currentUserHasReadAccess = ! $isGuestAccount && SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
			$currentUserHasModerateAccess = ! $isGuestAccount && SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
			$currentUserHasEditAccess = ! $isGuestAccount && SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
			$currentUserHasDeleteAccess = ! $isGuestAccount && ! $isAdminAccount && ! $isCurrentUser && SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN );
			...
			if ($isGuestAccount) {
				...
				foreach ( $userGroups as $userGroup ) {
					$userGroupsByKey [$userGroup ['gid']] = array (
							'gid' => $userGroup ['gid'] 
					);
				}
		...
		$pager = array (
				'numitems' => ModUtil::apiFunc ( $this->name, 'userusers', 'countItems', array (
						'letter' => $getAllArgs ['letter'] 
				) ),
				'itemsperpage' => $itemsPerPage 
		);
		
		// Assign the items to the template & return output
		return $this->view->assign ( 'usersitems', $userList )->assign ( 'pager', $pager )->assign ( 'allGroups', $groupsArray )->assign ( 'canSeeGroups', $canSeeGroups )->assign ( 'sort', $sort )->assign ( 'sortdir', $sortDirection )->assign ( 'userType', $userType )->assign ( 'available_options', $availableOptions )->fetch ( 'admin/Users/users_admin_view.tpl' );
	}
	public function viewOwner() {
		...
		PageUtil::setVar ( 'title', $this->__ ( 'View Owner' ) );
		$shop_id = $_REQUEST ['shop_id'];
		...
		// $allOwners = $repo->getShopOwners(array('shop_id' => $shop_id));
		// echo "<pre>"; print_r($allOwners); echo "</pre>";
		$userType = 'shopowner';
		
		$modvariable = $this->getVars ();
		$ownerGroup = $modvariable ['shopOwnerGroup'];
		// echo "Owner Group :". $ownerGroup;
		// we need this value multiple times, so we keep it
		// $itemsPerPage = $this->getVar(Users_Constant::MODVAR_ITEMS_PER_PAGE);
		...
		// Get all users as specified by the arguments.
		// $userList = ModUtil::apiFunc('ZSELEX', 'userusers', 'getAll', $getAllArgs);
		$userList = $repo->getShopOwners ( array (
				'shop_id' => $shop_id 
		) );
		...
		// Assign the items to the template & return output
		return $this->view->assign ( 'usersitems', $userList )->assign ( 'pager', $pager )->assign ( 'allGroups', $groupsArray )->assign ( 'canSeeGroups', $canSeeGroups )->assign ( 'sort', $sort )->assign ( 'sortdir', $sortDirection )->assign ( 'userType', $userType )->assign ( 'available_options', $availableOptions )->fetch ( 'admin/Users/users_owner_view.tpl' );
	}
	...
	public function newUser() {
		...
		if (! $this->getVar ( Users_Constant::MODVAR_REGISTRATION_ENABLED, false ) && ! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN )) {
		...
		if ($this->request->isPost ()) {
			...
			if (empty ( $errorFields ) && ! $validators->hasErrors ()) {
				...
				// echo "<pre>"; print_r($registeredObj); echo "</pre>"; exit;
				...
				if (isset ( $registeredObj ) && $registeredObj) {
					...
					$itemsGrp = array (
							'gid' => $shopAdminGroupId,
							'uid' => $newUserId 
					);
					
					$result = DBUtil::insertObject ( $itemsGrp, 'group_membership', true, 'gid' );
		...
		if ($proceedToForm) {
			return $this->view->assign_by_ref ( 'formData', $formData )->assign ( 'mode', 'new' )->assign ( 'errorMessages', $errorMessages )->assign ( 'errorFields', $errorFields )->fetch ( 'admin/Users/users_admin_newuser.tpl' );
		} else {
			$this->redirect ( ModUtil::url ( 'ZSELEX', 'adminusers', 'view', array (
					'shop_id' => $shop_id 
			) ) );
		}
	}
	public function newOwner() {
		...
		if ($ownerExist > 0) {
			...
			$this->redirect ( ModUtil::url ( 'ZSELEX', 'adminusers', 'viewOwner', array (
					'shop_id' => $shop_id 
			) ) );
		}
		...
		if ($this->request->isPost ()) {
			...
			$shopOwnerGroupId = $_REQUEST ['shopOwnerGroup'];
			...
			if (empty ( $errorFields ) && ! $validators->hasErrors ()) {
				...
				if (isset ( $registeredObj ) && $registeredObj) {
					...
					$whereOwner = "WHERE shop_id='" . $shop_id . "'";
					...
					DBUtil::insertObject ( $itemsOwner, 'zselex_shop_owners', true, 'id' ); // owner - shop
					
					$itemsGrp = array (
							'gid' => $shopOwnerGroupId,
							'uid' => $newUserId 
					);
		...
		if ($proceedToForm) {
			return $this->view->assign_by_ref ( 'formData', $formData )->assign ( 'mode', 'new' )->assign ( 'errorMessages', $errorMessages )->assign ( 'errorFields', $errorFields )->fetch ( 'admin/Users/users_admin_newowner.tpl' );
		} else {
			$this->redirect ( ModUtil::url ( 'ZSELEX', 'adminusers', 'viewOwner', array (
					'shop_id' => $shop_id 
			) ) );
	 ...
	 *        	Either 'search' or 'mailUsers', indicating which operation is calling this function.
	 *        	
	...
	protected function renderSearchForm($callbackFunc = 'search') {
		...
		return $this->view->assign ( 'groups', $groups )->assign ( 'callbackFunc', $callbackFunc )->fetch ( 'users_admin_search.tpl' );
	}
	...
	public function search() {
				...
				foreach ( $usersList as $key => $user ) {
					...
					if ($user ['uid'] != 1) {
						if (SecurityUtil::checkPermission ( $this->name . '::', $user ['uname'] . '::' . $user ['uid'], ACCESS_EDIT )) {
							$actions [$key] ['modifyUrl'] = ModUtil::url ( $this->name, 'admin', 'modify', array (
									'userid' => $user ['uid'] 
							) );
						}
						if (($currentUid != $user ['uid']) && SecurityUtil::checkPermission ( $this->name . '::', $user ['uname'] . '::' . $user ['uid'], ACCESS_DELETE )) {
							$actions [$key] ['deleteUrl'] = ModUtil::url ( $this->name, 'admin', 'deleteusers', array (
									'userid' => $user ['uid'] 
							) );
						}
					}
		...
		if (isset ( $usersList ) && $usersList) {
			return $this->view->assign ( 'items', $usersList )->assign ( 'actions', $actions )->assign ( 'deleteUsers', SecurityUtil::checkPermission ( $this->name . '::', '::', ACCESS_ADMIN ) )->fetch ( 'users_admin_search_results.tpl' );
	...
	public function mailUsers() {
		...
		if ($this->request->isPost ()) {
			...
			if ($formId == 'users_search') {
				...
				throw new Zikula_Exception_Fatal ( $this->__f ( 'An unknown form type was received by %1$s.', array (
						'mailUsers' 
				) ) );
			}
		} elseif (! $this->request->isGet ()) {
		...
		if ($this->request->isGet () || (($formId == 'users_search') && (! isset ( $userList ) || ! $userList)) || (($formId == 'users_mailusers') && ! $mailSent)) {
			...
			return $this->view->assign ( 'items', $userList )->assign ( 'mailusers', SecurityUtil::checkPermission ( $this->name . '::MailUsers', '::', ACCESS_COMMENT ) )->fetch ( 'users_admin_mailusers.tpl' );
		} elseif ($formId == 'users_mailusers') {
			$this->redirect ( ModUtil::url ( $this->name, 'admin', 'main' ) );
		} else {
			throw new Zikula_Exception_Fatal ( $this->__f ( 'The %1$s function has entered an unknown state.', array (
					'mailUsers' 
			) ) );
		}
	}
	...
	public function modify() {
		...
		if ($this->request->isPost ()) {
			...
			if (! $errorFields && ! $validators->hasErrors ()) {
				...
				if (isset ( $accessPermissions )) {
					...
					$groups = ModUtil::apiFunc ( 'Groups', 'user', 'getAll', array (
							'numitems' => 10000 
					) );
					$curUserGroupMembership = ModUtil::apiFunc ( 'Groups', 'user', 'getUserGroups', array (
							'uid' => $user ['uid'] 
					) );
			...
			$shop_id = $this->request->getGet ()->get ( 'shop_id', null );
		...
		if ($proceedToForm) {
			...
			if (! empty ( $accessPermissions )) {
				...
				$groupsUserMemberOf = ModUtil::apiFunc ( 'Groups', 'user', 'getusergroups', array (
						'uid' => $originalUser ['uid'] 
				) );
			...
			return $this->view->assign_by_ref ( 'formData', $formData )->assign ( 'user_attributes', $userAttributes )->assign ( 'defaultGroupId', ModUtil::getVar ( 'Groups', 'defaultgroup', 1 ) )->assign ( 'primaryAdminGroupId', ModUtil::getVar ( 'Groups', 'primaryadmingroup', 2 ) )->assign ( 'accessPermissions', $accessPermissions )->assign ( 'errorFields', $errorFields )->fetch ( 'admin/Users/users_admin_modify.tpl' );
		} else {
			$this->redirect ( ModUtil::url ( $this->name, 'adminusers', 'view', array (
					'shop_id' => $shop_id 
			) ) );
		}
	}
	public function modifyOwner() {
		...
		if ($proceedToForm) {
			...
			return $this->view->assign_by_ref ( 'formData', $formData )->assign ( 'user_attributes', $userAttributes )->assign ( 'defaultGroupId', ModUtil::getVar ( 'Groups', 'defaultgroup', 1 ) )->assign ( 'primaryAdminGroupId', ModUtil::getVar ( 'Groups', 'primaryadmingroup', 2 ) )->assign ( 'accessPermissions', $accessPermissions )->assign ( 'errorFields', $errorFields )->fetch ( 'admin/Users/users_admin_modifyowner.tpl' );
		} else {
			$this->redirect ( ModUtil::url ( $this->name, 'adminusers', 'viewOwner', array (
					'shop_id' => $shop_id 
			) ) );
		}
	}
	...
	public function lostUsername() {
		...
		if (! isset ( $uid ) || ! is_numeric ( $uid ) || (( int ) $uid != $uid) || ($uid <= 1)) {
			$this->registerError ( LogUtil::getErrorMsgArgs () )->redirect ( ModUtil::url ( $this->name, 'adminusers', 'view', array (
					'shop_id' => $shop_id 
			) ) );
		}
		...
		if (! $user) {
			$this->registerError ( $this->__ ( 'Sorry! Unable to retrieve information for that user id.' ) )->redirect ( ModUtil::url ( $this->name, 'adminusers', 'view', array (
					'shop_id' => $shop_id 
			) ) );
		}
		...
		if ($userNameSent) {
			$this->registerStatus ( $this->__f ( 'Done! The user name for \'%s\' has been sent via e-mail.', $user ['uname'] ) )->redirect ( ModUtil::url ( $this->name, 'admin', 'view' ) );
		} elseif (! $this->request->getSession ()->hasMessages ( Zikula_Session::MESSAGE_ERROR )) {
			$this->registerError ( $this->__f ( 'Sorry! There was an unknown error while trying to send the user name for \'%s\'.', $user ['uname'] ) )->redirect ( ModUtil::url ( $this->name, 'adminusers', 'view', array (
					'shop_id' => $shop_id 
			) ) );
		}
	}
	...
	public function deleteUsers($args) {
		// check permissions
		if (! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_DELETE )) {
			throw new Zikula_Exception_Forbidden ();
		}
		...
		if ($this->request->isPost ()) {
			$userSid = $this->request->getPost ()->get ( 'usersid', null );
			$shop_id = $this->request->getPost ()->get ( 'shop_id', null );
			...
			$userSid = $this->request->getGet ()->get ( 'usersid', null );
			$shop_id = $this->request->getPost ()->get ( 'shop_id', null );
		...
		if ($processDelete) {
			...
			foreach ( $userid as $uid ) {
				$event = new Zikula_Event ( 'module.users.ui.validate_delete', null, array (
						'id' => $uid 
			...
			if ($valid) {
				...
				$deleted = ModUtil::apiFunc ( $this->name, 'adminusers', 'deleteUser', array (
						'uid' => $userid 
				) );
				...
				if ($deleted) {
					...
					foreach ( $userid as $uid ) {
						$event = new Zikula_Event ( 'module.users.ui.process_delete', null, array (
								'id' => $uid 
						) );
						$this->eventManager->notify ( $event );
					...
					$count = count ( $userid );
					$this->registerStatus ( $this->_fn ( 'Done! Deleted %1$d user account.', 'Done! Deleted %1$d user accounts.', $count, array (
							$count 
					) ) );
				}
			}
		...
		if ($proceedToForm) {
			return $this->view->assign ( 'users', $users )->fetch ( 'admin/Users/users_admin_deleteusers.tpl' );
		} else {
			$this->redirect ( ModUtil::url ( $this->name, 'adminusers', 'view', array (
					'shop_id' => $shop_id 
			) ) );
	...
	public function deleteOwner($args) {
		// check permissions
		if (! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_DELETE )) {
			throw new Zikula_Exception_Forbidden ();
		}
			...
			if ($valid) {
				...
				if ($deleted) {
					...
					foreach ( $userid as $uid ) {
						...
						// $del1 = "DELETE FROM zselex_parent WHERE "
		...
		if ($proceedToForm) {
			return $this->view->assign ( 'users', $users )->fetch ( 'admin/Users/users_admin_deleteowner.tpl' );
		} else {
			$this->redirect ( ModUtil::url ( $this->name, 'adminusers', 'viewOwner', array (
					'shop_id' => $shop_id 
			) ) );
	...
	public function removeOwner($args) {
		$user_id = $this->request->getGet ()->get ( 'userid', '' );
		$shop_id = $this->request->getGet ()->get ( 'shop_id', '' );
		
		$where = "WHERE user_id=$user_id AND shop_id=$shop_id";
		// echo $where; exit;
		if (DBUtil::deleteWhere ( 'zselex_shop_owners', $where )) {
			$statement = Doctrine_Manager::getInstance ()->connection ();
			$modvars = ModUtil::getVar ( 'ZSELEX' );
			$repo = $this->entityManager->getRepository ( 'ZSELEX_Entity_Shop' );
			$count = $repo->getCount ( null, 'ZSELEX_Entity_ShopOwner', 'id', array (
					'a.co_owner' => 1 
			) );
			if ($count) {
				$ownerGroupId = $modvars ['shopOwnerGroup'];
				$dql = "DELETE FROM group_membership WHERE gid=$ownerGroupId AND uid=$user_id";
				// echo $dql;
				$query = $statement->execute ( $dql );
			}
			LogUtil::registerStatus ( $this->__ ( 'Done! Owner has been removed successfully.' ) );
			$this->redirect ( ModUtil::url ( $this->name, 'adminusers', 'viewOwner', array (
					'shop_id' => $shop_id 
			) ) );
		}
	}
	public function removeAdmin($args) {
		$user_id = $this->request->getGet ()->get ( 'userid', '' );
		$shop_id = $this->request->getGet ()->get ( 'shop_id', '' );
		
		$where = "WHERE user_id=$user_id AND shop_id=$shop_id";
		if (DBUtil::deleteWhere ( 'zselex_shop_admins', $where )) {
	 ...
	 *
	 * @param array $reglist
	 *        	The list of registration records.
	 * @param string $restoreView
	 *        	Indicates where the calling function expects to return to; 'view' indicates
	...
	protected function getActionsForRegistrations(array $reglist, $restoreView = 'view') {
		...
		if (! empty ( $reglist )) {
			...
			if (SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN )) {
				...
				foreach ( $reglist as $key => $reginfo ) {
					...
					$actions ['list'] [$reginfo ['uid']] = array (
							'display' => ModUtil::url ( $this->name, 'admin', 'displayRegistration', array (
									'uid' => $reginfo ['uid'] 
							) ),
							'modify' => ModUtil::url ( $this->name, 'admin', 'modifyRegistration', array (
									'uid' => $reginfo ['uid'],
									'restoreview' => $restoreView 
							) ),
							'verify' => $enableVerify ? ModUtil::url ( $this->name, 'admin', 'verifyRegistration', array (
									'uid' => $reginfo ['uid'],
									'restoreview' => $restoreView 
							) ) : false,
							'approve' => $enableApprove ? ModUtil::url ( $this->name, 'admin', 'approveRegistration', array (
									'uid' => $reginfo ['uid'] 
							) ) : false,
							'deny' => ModUtil::url ( $this->name, 'admin', 'denyRegistration', array (
									'uid' => $reginfo ['uid'] 
							) ),
							'approveForce' => $enableForced ? ModUtil::url ( $this->name, 'admin', 'approveRegistration', array (
									'uid' => $reginfo ['uid'],
									'force' => true 
							) ) : false 
					);
				}
			} elseif (SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_DELETE )) {
				$actions ['count'] = 5;
				foreach ( $reglist as $key => $reginfo ) {
			...
			} elseif (SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADD )) {
				$actions ['count'] = 4;
				foreach ( $reglist as $key => $reginfo ) {
			...
			} elseif (SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_EDIT )) {
				$actions ['count'] = 3;
				foreach ( $reglist as $key => $reginfo ) {
					$actionUrlArgs ['uid'] = $reginfo ['uid'];
					$enableVerify = ! $reginfo ['isverified'] && (($approvalOrder != Users_Constant::APPROVAL_BEFORE) || $reginfo ['isapproved']);
			...
			} elseif (SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_MODERATE )) {
				$actions ['count'] = 2;
				foreach ( $reglist as $key => $reginfo ) {
					$actionUrlArgs ['uid'] = $reginfo ['uid'];
					$enableVerify = ! $reginfo ['isverified'] && (($approvalOrder != Users_Constant::APPROVAL_BEFORE) || $reginfo ['isapproved']);
	...
	public function viewRegistrations() {
		...
		if ($backFromAction) {
			$returnArgs = $this->request->getSession ()->get ( 'Users_Controller_Admin_viewRegistrations', array (
					'startnum' => 1 
			), 'Zikula_Users' );
			...
			} elseif (! isset ( $returnArgs ['startnum'] ) || ! is_numeric ( $returnArgs ['startnum'] ) || empty ( $returnArgs ['startnum'] ) || (( int ) $returnArgs ['startnum'] != $returnArgs ['startnum']) || ($returnArgs ['startnum'] < 1)) {
			...
			if ($reset) {
				...
				$this->redirect ( ModUtil::url ( $this->name, 'admin1', 'viewRegistrations', $returnArgs ) );
			}
		}
		...
		$reglist = ModUtil::apiFunc ( $this->name, 'registration', 'getAll', array (
				'limitoffset' => $limitOffset,
				'limitnumrows' => $limitNumRows 
		) );
		...
		return $this->view->assign ( 'reglist', $reglist )->assign ( 'actions', $actions )->assign ( 'pager', $pager )->fetch ( 'users_admin_viewregistrations.tpl' );
	}
	...
	public function displayRegistration() {
		...
		if (empty ( $uid ) || ! is_numeric ( $uid )) {
			$this->registerError ( LogUtil::getErrorMsgArgs () );
			$this->redirect ( ModUtil::url ( $this->name, 'admin1', 'viewRegistrations', array (
					'return' => true 
			) ) );
		}
		...
		if (! $reginfo) {
			...
			$this->registerError ( $this->__ ( 'Unable to retrieve registration record. ' . 'The record with the specified id might not exist, or you might not have permission to access that record.' ) );
			return false;
		}
		...
		$actions = $this->getActionsForRegistrations ( array (
				$reginfo 
		), 'display' );
		
		return $this->view->assign ( 'reginfo', $reginfo )->assign ( 'actions', $actions )->fetch ( 'users_admin_displayregistration.tpl' );
	}
	...
	public function modifyRegistration() {
		if (! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_EDIT )) {
			throw new Zikula_Exception_Forbidden ();
		}
		...
		if ($this->request->isPost ()) {
			...
			if ($formData->isValid ()) {
				...
				$errorFields = ModUtil::apiFunc ( $this->name, 'registration1', 'getRegistrationErrors', $registrationArgs );
			} else {
				$errorFields = $formData->getErrorMessages ();
			}
			...
			if (! is_int ( $uid )) {
				if (! is_numeric ( $uid ) || (( string ) (( int ) $uid) != $uid)) {
					...
					$this->redirect ( ModUtil::url ( $this->name, 'admin', 'viewRegistrations', array (
							'restoreview' => true 
					) ) );
				}
			}
			
			$registration = ModUtil::apiFunc ( $this->name, 'registration1', 'get', array (
					'uid' => $uid 
			) );
		...
		if ($proceedToForm) {
			...
			return $this->view->assign_by_ref ( 'formData', $formData )->assign ( $rendererArgs )->fetch ( 'users_admin_modifyregistration.tpl' );
		} else {
			if ($restoreView == 'view') {
				$this->redirect ( ModUtil::url ( $this->name, 'admin1', 'viewRegistrations1', array (
						'restoreview' => true 
				) ) );
			} else {
				$this->redirect ( ModUtil::url ( $this->name, 'admin1', 'displayRegistration1', array (
	...
	public function approveRegistration() {
		...
		if ($reginfo ['isapproved'] && ! $forceVerification) {
			...
			$this->redirect ( $cancelUrl );
		} elseif (! $forceVerification && ($approvalOrder == Users_Constant::APPROVAL_AFTER) && ! $reginfo ['isapproved'] && ! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN )) {
		...
		if (! $confirmed) {
			...
			return $this->view->assign ( 'reginfo', $reginfo )->assign ( 'restoreview', $restoreView )->assign ( 'force', $forceVerification )->assign ( 'cancelurl', $cancelUrl )->fetch ( 'users_admin_approveregistration.tpl' );
		} else {
			$this->checkCsrfToken ();
	...
	public function denyRegistration() {
		...
		// Got just a uid.
		$reginfo = ModUtil::apiFunc ( $this->name, 'registration', 'get', array (
				'uid' => $uid 
		) );
		if (! $reginfo) {
			$this->registerError ( $this->__f ( 'Error! Unable to retrieve registration record with uid \'%1$s\'', $uid ) );
		...
		if ($restoreView == 'display') {
			$cancelUrl = ModUtil::url ( $this->name, 'admin', 'displayRegistration', array (
					'uid' => $reginfo ['uid'] 
			) );
		} else {
			$cancelUrl = ModUtil::url ( $this->name, 'admin', 'viewRegistrations', array (
					'restoreview' => true 
			) );
		}
		...
		if (! $confirmed) {
			...
			return $this->view->assign ( 'reginfo', $reginfo )->assign ( 'restoreview', $restoreView )->assign ( 'cancelurl', $cancelUrl )->fetch ( 'users_admin_denyregistration.tpl' );
		} else {
	...
	public function config() {
		...
		if ($this->request->isPost ()) {
			...
			if ($configData->isValid ()) {
				...
				$this->registerError ( $this->_fn ( 'There was a problem with one of the module settings. Please review the message below, correct the error, and resubmit your changes.', 'There were problems with %1$d module settings. Please review the messages below, correct the errors, and resubmit your changes.', $errorCount, array (
						$errorCount 
				) ) );
			}
		} elseif (! $this->request->isGet ()) {
		...
		return $this->view->assign_by_ref ( 'configData', $configData )->assign ( 'errorFields', $errorFields )->fetch ( 'users_admin_config.tpl' );
	}
	...
	public function import($args) {
		...
		return $this->view->assign ( 'importResults', isset ( $importResults ) ? $importResults : '' )->assign ( 'post_max_size', $post_max_size )->assign ( 'minpass', $minpass )->assign ( 'defaultGroup', $defaultGroup )->fetch ( 'users_admin_import.tpl' );
	}
	...
	/**
	 * Import several users from a CSV file.
	 * Checks needed values and format.
	 *
	 * Parameters passed via GET:
	 * --------------------------
	 * None.
	 ...
	 *        	Information about the file to import. Used as the default
	 ...
	 *        	A code indicating the delimiter used in the file. Used as the
	...
	protected function uploadImport(array $importFile, $delimiter) {
		...
		foreach ( $lines as $line_num => $line ) {
			...
			if ($counter == 0) {
				...
				foreach ( $firstLineArray as $field ) {
					if (! in_array ( trim ( strtolower ( $field ) ), $expectedFields )) {
						return $this->__f ( "Error! The import file does not have the expected field %s in the first row. Please check your import file.", array (
								$field 
						) );
					}
				}
			...
			if ($uname == '' || strlen ( $uname ) > 25) {
				return $this->__f ( 'Sorry! The user name is not valid in line %s. The user name is mandatory and the maximum length is 25 characters. Please check your import file.', $counter );
			}
			...
			if (! $is_admin && $pregcondition != '') {
				...
				if (preg_match ( $pregcondition, $uname )) {
					return $this->__f ( 'Sorry! The user name %1$s is reserved and cannot be registered in line %2$s. Please check your import file.', array (
							$uname,
							$counter 
					) );
				}
			...
			if (preg_match ( "/[[:space:]]/", $uname ) || ! System::varValidate ( $uname, 'uname' )) {
				return $this->__f ( 'Sorry! The user name %1$s cannot contain spaces in line %2$s. Please check your import file.', array (
						$uname,
						$counter 
				) );
			}
			...
			if (in_array ( $uname, $usersArray )) {
				return $this->__f ( 'Sorry! The user name %1$s is repeated in line %2$s, and it cannot be used twice for creating accounts. Please check your import file.', array (
						$uname,
						$counter 
				) );
			}
			...
			if (strlen ( $pass ) < $minpass) {
				return $this->__f ( 'Sorry! The password must be at least %1$s characters long in line %2$s. Please check your import file.', array (
						$minpass,
						$counter 
				) );
			}
			...
			if ($reg_uniemail == 1) {
				if (in_array ( $email, $emailsArray )) {
					return $this->__f ( 'Sorry! The %1$s e-mail address is repeated in line %2$s, and it cannot be used twice for creating accounts. Please check your import file.', array (
							$email,
							$counter 
					) );
				}
			...
			if ($groups == '') {
				...
				foreach ( $groupsArray as $group ) {
					if (! in_array ( $group, $allGroupsArray )) {
						return $this->__f ( 'Sorry! The identity of the group %1$s is not not valid in line %2$s. Perhaps it do not exist. Please check your import file.', array (
								$group,
								$counter 
						) );
					}
		...
		$usersInDB = ModUtil::apiFunc ( $this->name, 'admin', 'checkMultipleExistence', array (
		...
		if ($reg_uniemail == 1) {
			$emailsInDB = ModUtil::apiFunc ( $this->name, 'admin', 'checkMultipleExistence', array (
					'valuesarray' => $emailsArray,
					'key' => 'email' 
			) );
			if ($emailsInDB === false) {
		...
		if (! ModUtil::apiFunc ( $this->name, 'admin', 'createImport', array (
	...
	public function toggleForcedPasswordChange() {
		...
		if ($this->request->isGet ()) {
			...
			return $this->view->assign ( 'user_obj', $userObj )->assign ( 'user_must_change_password', $userMustChangePassword )->fetch ( 'users_admin_toggleforcedpasswordchange.tpl' );
		} elseif ($this->request->isPost ()) {
			$this->checkCsrfToken ();
			...
			if ($userMustChangePassword) {
				if (isset ( $userObj ['__ATTRIBUTES__'] ) && isset ( $userObj ['__ATTRIBUTES__'] ['_Users_mustChangePassword'] )) {
					$this->registerStatus ( $this->__f ( 'Done! A password change will be required the next time %1$s logs in.', array (
							$userObj ['uname'] 
					) ) );
				} else {
					throw new Zikula_Exception_Fatal ();
				...
				if (isset ( $userObj ['__ATTRIBUTES__'] ) && isset ( $userObj ['__ATTRIBUTES__'] ['_Users_mustChangePassword'] )) {
					...
					$this->registerStatus ( $this->__f ( 'Done! A password change will no longer be required for %1$s.', array (
							$userObj ['uname'] 
					) ) );
				}
			}
