 ...
 */
class ZSELEX_Controller_Ajaxuser extends Zikula_Controller_AbstractAjax {
	/**
	...
	public function getUsers() {
		...
		if (SecurityUtil::checkPermission ( 'Users::', '::', ACCESS_MODERATE )) {
			...
			if (! SecurityUtil::checkPermission ( 'News::', '::', ACCESS_ADMIN )) {
				$extra = " AND uid IN (SELECT parentId FROM zselex_parent WHERE childType='SHOP' AND parentType='SHOPADMIN' AND cr_uid='" . UserUtil::getVar ( 'uid' ) . "')";
				$where .= $extra;
			} //
	...
	public function getRegistrationErrors() {
		...
		if (($eventType == 'new_registration') && ! $this->getVar ( 'reg_allowreg', true ) && ! SecurityUtil::checkPermission ( 'Users::', 'user::createshopadmin', ACCESS_ADMIN )) {
		...
		if ($totalErrors > 0) {
			$returnValue ['errorMessages'] [] = $this->_fn ( 'There was an error with one of the fields, below. Please review the message, and correct your entry.', 'There were errors with %1$d of the fields, below. Please review the messages, and correct your entries.', $totalErrors, array (
					$totalErrors 
			) );
	...
	public function getLoginFormFields() {
		...
		if (empty ( $modname ) || ! is_string ( $modname )) {
			...
			throw new Zikula_Exception_Fatal ( $this->__f ( 'The \'%1$s\' module is not in an available state.', array (
					$modname 
			) ) );
		} elseif (! ModUtil::isCapable ( $modname, 'authentication' )) {
			throw new Zikula_Exception_Fatal ( $this->__f ( 'The \'%1$s\' module is not an authentication module.', array (
					$modname 
			) ) );
		}
