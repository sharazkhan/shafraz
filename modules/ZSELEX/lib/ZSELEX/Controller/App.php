...
class ZSELEX_Controller_App extends ZSELEX_Controller_Base_User
{
    ...
    public function locate()
    {
        ...
        if (!empty($Country)) {
            ...
            /*
             * $countryobj = ModUtil::apiFunc('ZSELEX', 'user', 'get', $args = array(
             * 'table' => 'zselex_country',
             * 'where' => "country_name LIKE '%" . DataUtil::formatForStore($Country) . "%' ",
             * 'fields' => array('country_id', 'country_name')
             * ));
             */

            $countryobj = $countryRepo->getAppCountry(array(
                'country' => $Country
                ));
        ...
        if ($Level == 'street' || $Level == 'shop') {
            if (!empty($Adress)) {
                ...
                $addrobj = $countryRepo->getAppShopAddress(array(
                    'addressQry' => $AdressLike
                    ));
    ...
    function getAppRegion($Region, $type)
    {
        ...
        $regionobj = $repo->getAppRegion(array(
            'regionQry' => $RegionLike
            ));
        ...
        if ($cityobj == true) {
            ...
            if ($city_id > 0) {
                ...
                if ($_SESSION ['region_cookie'] < 1) {
                    setcookie("region_cookie", $cityobj ['region_id'],
                        time() + 604800, '/');
                    setcookie("regionname_cookie", $cityobj ['region_name'],
                        time() + 604800, '/');
                }
            }
        }
    ...
    function getAppArea($Area)
    {
        ...
        if ($area_id > 0) {
            ...
            if ($_SESSION ['region_cookie'] < 1) {
                setcookie("region_cookie", $areaobj ['region_id'],
                    time() + 604800, '/');
                setcookie("regionname_cookie", $areaobj ['region_name'],
                    time() + 604800, '/');
            }
            if ($_SESSION ['city_cookie'] < 1) {
                setcookie("city_cookie", $areaobj ['city_id'], time() + 604800,
                    '/');
                setcookie("cityname_cookie", $areaobj ['city_name'],
                    time() + 604800, '/');
            }
        }
        return true;
