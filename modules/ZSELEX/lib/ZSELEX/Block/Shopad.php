 ...
 */
class ZSELEX_Block_Shopad extends Zikula_Controller_AbstractBlock {
	public $amount;
	...
	public function init() {
		SecurityUtil::registerPermissionSchema ( 'ZSELEX:shopad:', 'Block title::' );
	}
	...
	public function info() {
		return array (
				'text_type' => 'selection',
				'module' => 'ZSELEX',
				'text_type_long' => $this->__ ( 'Shops AD Block' ),
				'allow_multiple' => true,
				'form_content' => false,
				'form_refresh' => false,
				'show_preview' => true,
	...
	public function display($blockinfo) {
		// return;
		if (! SecurityUtil::checkPermission ( 'ZSELEX:shopad:', "$blockinfo[title]::", ACCESS_OVERVIEW )) {
			return;
		}
		...
		$sql = "SELECT s.* FROM zselex_shop AS s WHERE s.shop_id IS NOT NULL $limit";
		
		$items = ModUtil::apiFunc ( 'ZSELEX', 'admin', 'execteQuery', $sql );
		// $countQuery = " SELECT COUNT(s.shop_id) AS count FROM zselex_shop AS s WHERE s.shop_id IS NOT NULL LIMIT 0,".$vars['amount'];
		// $total_shops = ModUtil::apiFunc('ZSELEX', 'admin', 'execteQuery', $countQuery);
		// print_r($total_shops);
		
		$this->view->assign ( 'vars', $vars );
		$this->view->assign ( 'shops', $items );
		// $this->view->assign('shopconfig', $shopconfig);
		
		$blockinfo ['content'] = $this->view->fetch ( 'blocks/shops_ad.tpl' );
	...
	public function modify($blockinfo) {
		...
		return $this->view->fetch ( 'blocks/shops_ad_modify.tpl' );
	}
	...
	public function update($blockinfo) {
		...
		$this->view->clear_cache ( 'blocks/shops_ad_modify.tpl' );
