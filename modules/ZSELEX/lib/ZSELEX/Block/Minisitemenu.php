 ...
 */
class ZSELEX_Block_Minisitemenu extends Zikula_Controller_AbstractBlock {
	
	/**
	 * initialise block
	 */
	public function init() {
		SecurityUtil::registerPermissionSchema ( 'Minisitemenu::', 'Block ID:Link ID:' );
	}
	...
	public function info() {
		return array (
				'module' => $this->name,
				'text_type' => $this->__ ( 'Minisitemenu Extended Menu' ),
				'text_type_long' => $this->__ ( 'Minisitemenu Extended menu block' ),
				'allow_multiple' => true,
				'form_content' => false,
				'form_refresh' => false,
				'show_preview' => true,
	...
	public function display($blockinfo) {
		...
		// security check
		// echo $blockinfo['bid'];
		if (! SecurityUtil::checkPermission ( 'Minisitemenu::', $blockinfo ['bid'] . '::', ACCESS_READ )) {
			return;
		}
		...
		if (empty ( $vars ['template'] )) {
			$vars ['template'] = 'blocks/minisitemenu/blocks_block_minisiteextmenu.tpl';
		}
		// stylesheet to use
		...
		// echo "<pre>"; print_r($blockinfo['title']); echo "</pre>";
		// echo "<pre>"; print_r($vars['links'][$thislang]); echo "</pre>";
		...
		if (! empty ( $vars ['links'] [$thislang] )) {
			...
			foreach ( $vars ['links'] [$thislang] as $linkid => $link ) {
				...
				// echo $linkid;
				$denied = ! SecurityUtil::checkPermission ( 'Minisitemenu::', $blockinfo ['bid'] . ':' . $linkid . ':', ACCESS_READ );
				if ($denied || (! is_null ( $link ['parentid'] ) && in_array ( $link ['parentid'], $blocked ))) {
					...
					// echo $str . '<br>';
					$link ['url'] = $link ['url'] . ":shop_id=$shop_id" . "}";
					// $link['url'] = ModUtil::apiFunc('Blocks', 'user', 'encodebracketurl', $link['url']);
					$link ['url'] = $this->encodebracketurl ( $link ['url'] );
					// echo $link['url'] . '<br>';
					// $link['url'] = $link['url'] . '&shop_id=' . $shop_id;
					// echo $link['url'] . '<br>';
		...
		if (! empty ( $vars ['displaymodules'] )) {
			...
			if (count ( $menuitems ) > 0) {
				...
				if (SecurityUtil::checkPermission ( 'Minisitemenu::', $blockinfo ['bid'] . '::', ACCESS_ADMIN )) {
					$menuitems [] = array (
							'name' => $this->__ ( '--Installed modules--' ),
							'url' => ModUtil::url ( 'Blocks', 'admin', 'modify', array (
									'bid' => $blockinfo ['bid'] 
							) ),
							'title' => '',
							'level' => 0,
		...
		foreach ( $menuitems as $k => $v ) {
			...
			$url1 = explode ( '/', $v [url] );
			$menuitems [$k] ['func'] = $url1 [0];
			if (strpos ( $v [url], 'shopsummary' ) !== false) {
			...
			foreach ( $services as $ser ) {
				...
				if (! empty ( $ser )) {
					...
					$servicePerms = ModUtil::apiFunc ( 'ZSELEX', 'admin', 'servicePermission', $args = $serviceargs );
					// echo "<pre>"; print_r($servicePerm); echo "</pre>";
					if ($servicePerms ['perm'] < 1) {
						$cant ++;
					}
				}
			}
		...
		if (((SecurityUtil::checkPermission ( 'ZSELEX::', '::', ACCESS_ADMIN )) || (UserUtil::getVar ( 'uname' ) == $ownername)) && $_REQUEST ['type'] == 'user') {
			// array_push($menuitems, array('name' => 'Administration', 'func' => 'shopsummary', 'type' => $_REQUEST['type'], 'url' => ModUtil::url('zselex', 'admin', 'shopsummary', array('shop_id' => $shop_id))));
		}
		// echo "<pre>"; print_r($menuitems); echo "</pre>";
		// echo $uri;
		// echo $currenturi;
		// echo $currenturi = $_REQUEST['func'] . '/' . $_REQUEST['shop_name'] . '/';
		// assign the items
		$this->view->assign ( 'menuitems', $menuitems )->assign ( 'blockinfo', $blockinfo )->assign ( 'currenturi', $currenturi )->assign ( 'functions', $functions )->assign ( 'access_edit', Securityutil::checkPermission ( 'Minisitemenu::', $blockinfo ['bid'] . '::', ACCESS_EDIT ) );
	...
	/**
	 * do a simple check .
	 * . to see if the current URL is the menu item
	 *
	 * @param
	 *        	none
	 * @return boolean
	...
	public function modify($blockinfo) {
		...
		if ($addurl == 1) {
			...
			foreach ( $languages as $singlelanguage ) {
				$vars ['links'] [$singlelanguage] [] = array (
		...
		// echo "<pre>"; print_r($vars['links']); echo "</pre>";
		...
		// return the output
		return $this->view->fetch ( 'blocks/minisitemenu/blocks_block_minisiteextmenu_modify.tpl' );
	}
