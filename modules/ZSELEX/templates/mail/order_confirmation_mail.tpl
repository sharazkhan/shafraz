...
<h3>{gt text='Your Order Id'} : {$order_id}</h3>
{gt text='Congratulations on your order'}
<h4>{gt text='Payment Method'} : {$payment_method}</h4>
{if $payment_mode.test_mode}
<h4>{gt text='Payment Mode'} : {gt text='Test'} </h4>
{/if}
{if $cardtype neq ''}
<h4>{gt text='Card Type'} : {$cardtype}</h4> 
{/if}
<h3><u>{gt text='Your Details'}</u> :</h3> 
...
{if $payment_method eq 'directpay'}
    <br><br>
    {$checkout_info.info}
 {/if}   
      <br><br>
 <div><b>{gt text='Shop'}: &nbsp; {$shop_info.shop_name}</b> </div>
 <div>{$shop_info.address|nl2br}</div>
 <div>{gt text='Phone'}: &nbsp; {$shop_info.telephone}</div>
 <div>{gt text='Email'}: &nbsp; {$shop_info.email}</div>
