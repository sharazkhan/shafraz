...
{if $notupdate_recent eq 1}
    <span>
   {gt text="You have not updated in recent time"}&nbsp;&nbsp;&nbsp;<a href="{modurl modname='ZSELEX' type='admin' func='shopinnerview' shop_id=$smarty.request.shop_id op='cl'}">{gt text="clear this"}</a>
    </span>
{/if} 
...
<tr><td>
    ...
    {if $perm eq 1}
    <a href='{modurl  modname='ZSELEX' type='user' func='shop' shop_id=$smarty.request.shop_id}'>{$item.shop_name|cleantext|upper} </a>
    {else}
    {$miniShopLinkStrt}{$item.shop_name|cleantext|upper}{$miniShopLinkEnd}   
    {/if}    
</td></tr>
<tr><td>
