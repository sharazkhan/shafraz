...
<script>
 ...
 function downgradeConfirm(){
    if(confirm(Zikula.__("Are you sure you want to Downgrade?"))==true)
        return true;
    else
        return false;
}
                    ...
                    {else}<!-- if bundle exist -->
                        ...
                        {else}
                              {if $existing_bundle.bundle_id neq $bundle.bundle_id}
                                 <a href="javascript:document.servicecart{$bundle.bundle_id}.submit()" onclick="return downgradeConfirm()">
                                    <button class="MyserviceBtn">
                                        {gt text='Downgrade'} <img src="{$themepath}/images/ArrowShadow.png" />
                                    </button>
                                </a>
                              {/if}
                        {/if} <!-- button show -->
