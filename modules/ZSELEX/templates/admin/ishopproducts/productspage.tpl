...
{jscroll file_name='modules/ZSELEX/javascript/product_scroll.js'}
...
<script>
     ...
     // editProduct('218');
      ...
      {{if $smarty.request.product_id > 0}}
      jQuery( window ).load(function() {
      //alert('hiii');
      //  editProduct('{{$smarty.request.product_id}}' , '{{$smarty.request.src}}')
       editProduct('{{$smarty.request.product_id}}' , '{{$source}}')
      });
      {{/if}}
...
{if !$service.expired AND !$service.disable AND $qtyLeft > 0}
...
{if !$service.expired AND !$service.disabled}
...
{if $smarty.request.manage neq ''}
  ...
  <iframe id="iFrameWrapper" src="{modurl modname="ZSELEX" type='admin' func='manufacturers' shop_id=$smarty.request.shop_id}" width="100%" frameborder="0"></iframe>
   ...
   <iframe id="iFrameWrapper" src="{modurl modname="ZSELEX" type='admin' func='productOption' shop_id=$smarty.request.shop_id}" width="100%" frameborder="0"></iframe>
     {elseif $smarty.request.manage eq 'discount'}
   <iframe id="iFrameWrapper" src="{modurl modname="ZSELEX" type='admin' func='discount' shop_id=$smarty.request.shop_id}" width="100%" frameborder="0"></iframe>
    {/if}
...
*}
 <button onClick="editSettings()" class="ProductPageBtn"  type="button">{gt text="Settings"} </button>
 <a href="{modurl modname="ZSELEX" type='admin' func='products' shop_id=$smarty.request.shop_id manage="prod_cat"}">
  ...
  <a href="{modurl modname="ZSELEX" type='admin' func='products' shop_id=$smarty.request.shop_id manage="discount"}">
     <button class="ProductPageBtn"  type="button"> {gt text="Discounts"} </button>
 </a>
            ...
            <tbody>
                ...
                <tr id="{$item.product_id}" onClick="editProduct('{$item.product_id}')" style="cursor:pointer" class="{cycle values='z-odd,z-even'} prodTr prodTrs{$item.product_id}">
                   ...
                   <td>
                    ...
                    <td>{displayprice amount=$item.prd_price}</td>
...
{pager rowcount=$total_products limit=$itemsperpage posvar='startnum' maxpages=10}
