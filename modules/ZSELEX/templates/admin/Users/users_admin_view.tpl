{ajaxheader modname=$modinfo.name filename='users1.js' ui=true}
{strip}
...
{
if(confirm(Zikula.__("Do you really want to remove this admin from the shop?"))==true)
return true;
else
return false;
}
...
{modfunc modname=ZSELEX type=admin func=shopheader}
...
<div id="liveusersearch" class="z-hide z-form">
    ...
    <fieldset>
        ...
        *}
        <div class="z-admin-content-pagetitle">
    <a href="index.php?module=zselex&type=adminusers&func=newUser&shop_id={$smarty.request.shop_id}" class="z-iconlink z-icon-es-add">{gt text="Create admin for this shop"}</a>
       </div>
         <div class="z-admin-content-pagetitle">
    <a href="index.php?module=zselex&type=admin&func=existingAdmin&shop_id={$smarty.request.shop_id}" class="z-iconlink z-icon-es-add">{gt text="Assign from existing admins"}</a>
        </div>
 {securityutil_checkpermission_block component='ZSELEX::' instance='::' level=ACCESS_ADMIN}
         <div class="z-admin-content-pagetitle">
    <a href="index.php?module=zselex&type=admin&func=existingAdminAll&shop_id={$smarty.request.shop_id}" class="z-iconlink z-icon-es-add">{gt text="Assign from all admins"}</a>
        </div>
 {/securityutil_checkpermission_block}
 
  <div class="z-admin-content-pagetitle">
    <a href="index.php?module=zselex&type=admin&func=existingAdmin&shop_id={$smarty.request.shop_id}&assign=coowner" class="z-iconlink z-icon-es-add">{gt text="Assign Co-Owner"}</a>
  </div>
...
<p id="users-alphafilter">
   {* <strong>[{pagerabc posvar="letter" forwardvars="sortby" shop_id=$smarty.request.shop_id}]</strong> *}
</p>
...
<table class="z-datatable">
    <thead>
        <tr>
            ...
            <th class="z-center">
                {sortlink __linktext='Status' sort='activated' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='view' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='User name' sort='uname' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='view' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='Internal ID' sort='uid' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='view' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='Registration date' sort='user_regdate' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='view' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='Last login' sort='lastlogin' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='view' shop_id=$smarty.request.shop_id}
            </th>
            {if $canSeeGroups}
    ...
    <tbody class="z-clearer">
        ...
        <tr class="{cycle values='z-odd,z-even'}">
            ...
            <td class="users_action">
                ...
                <a href="{modurl modname='ZSELEX' type='adminusers' func='lostUsername' shop_id=$smarty.request.shop_id userid=$usersitems[usersitems].uid csrftoken=$csrftoken}">{img modname='core' set='icons/extrasmall' src='lostusername.png' title=$title alt=$title class='tooltips'}</a>
                {else}
            ...
            <td class="users_action">
                ...
                <a href="{modurl modname='ZSELEX' type='adminusers' func='lostPassword' shop_id=$smarty.request.shop_id  userid=$usersitems[usersitems].uid csrftoken=$csrftoken}">{img modname='core' set='icons/extrasmall' src='lostpassword.png' title=$title alt=$usersitems[usersitems].options.lostPassword.title class='tooltips'}</a>
                {else}
            ...
            <td class="users_action">
                ...
                {/if}
                <a href="{modurl modname='ZSELEX' type='adminusers' func='toggleForcedPasswordChange' shop_id=$smarty.request.shop_id userid=$usersitems[usersitems].uid}">{img modname='core' set='icons/extrasmall' src=$image title=$title alt=$title class='tooltips'}</a>
                {else}
                {assign var='image' value='password_expire.png'}
            ...
            <td class="users_action">
                ...
                <a href="{modurl modname='ZSELEX' type='adminusers' func='modify' userid=$usersitems[usersitems].uid shop_id=$smarty.request.shop_id}">{img modname='core' set='icons/extrasmall' src='xedit.png' title=$title alt=$title class='tooltips'}</a>
                {else}
            ...
            <td class="users_action">
                ...
                <a href="{modurl modname='ZSELEX' type='adminusers' func='deleteUsers' userid=$usersitems[usersitems].uid shop_id=$smarty.request.shop_id}">{img modname='core' set='icons/extrasmall' src='14_layer_deletelayer.png' title=$title alt=$title class='tooltips'}</a>
                {else}
                ...
                {/if}
                  <a href="{modurl modname='ZSELEX' type='adminusers' func='removeAdmin' userid=$usersitems[usersitems].uid shop_id=$smarty.request.shop_id}">
             <img src="{$baseurl}images/icons/extrasmall/delete_user.png" alt="{gt text='Remove this admin'}" title="{gt text='Remove this owner from this shop'}" class='tooltips' onclick='return confirmation()'>
                 </a>
            </td>
            {/if}
...
{modfunc modname=admin type=admin func=adminfooter}