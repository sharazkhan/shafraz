...
{
if(confirm(Zikula.__("Do you really want to remove this owner from the shop?"))==true)
return true;
else
return false;
}
...
<div id="liveusersearch" class="z-hide z-form">
    ...
    <fieldset>
        ...
        *}
        <div class="z-admin-content-pagetitle">
    <a href="index.php?module=zselex&type=adminusers&func=newOwner&shop_id={$smarty.request.shop_id}" class="z-iconlink z-icon-es-add">{gt text='Create new owner for this shop'}</a>
        </div>
        
         <div class="z-admin-content-pagetitle">
    <a href="index.php?module=zselex&type=admin&func=existingOwner&shop_id={$smarty.request.shop_id}" class="z-iconlink z-icon-es-add">{gt text='Assign from existing owners'}</a>
        </div>
...
<table class="z-datatable">
    <thead>
        <tr>
            ...
            <th class="z-center">
                {sortlink __linktext='Status' sort='activated' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='viewOwner' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='User name' sort='uname' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='viewOwner' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='Internal ID' sort='uid' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='viewOwner' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='Registration date' sort='user_regdate' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='viewOwner' shop_id=$smarty.request.shop_id}
            </th>
            <th>
                {sortlink __linktext='Last login' sort='lastlogin' currentsort=$sort sortdir=$sortdir modname='ZSELEX' type='adminusers' func='viewOwner' shop_id=$smarty.request.shop_id}
            </th>
            {if $canSeeGroups}
    ...
    <tbody class="z-clearer">
        ...
        <tr class="{cycle values='z-odd,z-even'}">
            ...
            <td class="users_action">
                ...
                <a href="{modurl modname='ZSELEX' type='adminusers' func='modifyOwner' userid=$usersitems[usersitems].uid shop_id=$smarty.request.shop_id}">{img modname='core' set='icons/extrasmall' src='xedit.png' title=$title alt=$title class='tooltips'}</a>
                {else}
            ...
            <td class="users_action">
                ...
                <a href="{modurl modname='ZSELEX' type='adminusers' func='deleteOwner' userid=$usersitems[usersitems].uid shop_id=$smarty.request.shop_id}">{img modname='core' set='icons/extrasmall' src='14_layer_deletelayer.png' title=$title alt=$title class='tooltips'}</a>
                {else}
                ...
                {/if}
                <a href="{modurl modname='ZSELEX' type='adminusers' func='removeOwner' userid=$usersitems[usersitems].uid shop_id=$smarty.request.shop_id}">
             <img src="{$baseurl}images/icons/extrasmall/delete_user.png" alt="{gt text='Remove this owner'}" title="{gt text='Remove this owner from this shop'}" class='tooltips' onclick='return confirmation()'>
                 </a>
            </td>
            {/if}
