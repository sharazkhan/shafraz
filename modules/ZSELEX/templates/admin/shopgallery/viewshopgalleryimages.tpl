            ...
            <tbody>
                ...
                <tr class="{cycle values='z-odd,z-even'}">
                   ...
                   <td>
                    ...
                    <td><input type="radio" name="defaultimage" id="image{$item.gallery_id}" {if $item.defaultImg eq 1} checked="checked" {/if} value="{$item.gallery_id}" />  <label for="image{$item.gallery_id}">{gt text='Default'}</label></td>
                    
                    <td>{$item.image_description|safetext}</td>
                    <td><a href='{modurl modname='ZSELEX' type='admin' func='modifyshopgalleryimages' id=$item.gallery_id shop_id=$item.shop_id}'>{gt text='Edit'}</a></td>
