...
<div class="z-admin-content-pagetitle">
    <a href="index.php?module=zselex&amp;type=admin&amp;func=createregion" class="z-iconlink z-icon-es-add">{gt text='Create Region'}</a>
</div>
 {/securityutil_checkpermission_block}
<div class="z-admin-content-pagetitle">
    {icon type="view" size="small"}
    <h3>{gt text='New regions list'}</h3>
</div>

<form class="z-form" id="country_filter" action="{modurl modname='ZSELEX' type='admin' func='viewregion'}" method="post" enctype="application/x-www-form-urlencoded">
    <fieldset id="zselex_multicategory_filter"{if $filter_active} class='filteractive'{/if}>
        ...
        <legend>{gt text='Filter %1$s, %2$s country listed' plural='Filter %1$s, %2$s regions listed' count=$total_regions tag1=$filteractive tag2=$total_regions}</legend>
        <input type="hidden" name="startnum" value="{$startnum}" />
        <input type="hidden" name="order" value="{$order}" />
        <input type="hidden" name="sdir" value="{$sdir}" />
        <label for="searchtext">{gt text='Region Name'}</label>
     ...
     <div style="overflow:auto;">
       ...
       <table id="zselex_admintable" class="z-datatable">
            <thead>
                <tr>
                   <!--   <th></th> -->
                    <th><a class='{$sort.class.region_id}' href='{$sort.url.region_id|safetext}'>{gt text='ID'}</a></th>
            ...
            <tbody>
                {foreach from=$regionsitems item='regionitem'}
                <tr class="{cycle values='z-odd,z-even'}">
                   <!--   <td><input type="checkbox" name="zselex_selected_regions[]" value="{$regionitem.region_id}" class="zselex_checkbox" /></td> -->
                    <td>{$regionitem.region_id|safetext}</td>
                    <td>{$regionitem.region_name|safetext}</td>
                    <td>{$regionitem.description|safetext}</td>
                    <td>
                   {$regionitem.country_name|safetext}
                     </td> 

                    <td>{$aStatus[$regionitem.status]|safetext}</td>
                    <td>{$regionitem.createduser|safetext}</td>
                    <td>{$regionitem.cr_date|safetext}</td>
                    <td>{$regionitem.updateduser|safetext}</td>
                    <td>{$regionitem.lu_date|safetext}</td>
                    <td>
                        {assign var='options' value=$regionitem.options}
                ...
                {foreachelse}
                <tr class="z-datatableempty"><td colspan="10">{gt text='No regions currently in database.'}</td></tr>
                {/foreach}
            </tbody>
        </table>
...
<form class="z-form" action="{modurl modname='ZSELEX' type='admin' func='modifycountry'}" method="post">
    <div>
        <fieldset>
            <label for="directregion_id">{gt text='Access a past country via its ID'}:</label>
            <input type="text" id="directregion_id" name="region_id" value="" size="5" maxlength="8" />
            <span class="z-nowrap z-buttons">
...
-->
{pager rowcount=$total_regions limit=$itemsperpage posvar='startnum' maxpages=10}
