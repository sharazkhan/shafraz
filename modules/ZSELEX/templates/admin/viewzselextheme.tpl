...
<div class="z-admin-content-pagetitle">
       <a href="index.php?module=zselex&amp;type=admin&amp;func=configurethemezselex">{gt text='Configure Theme for ZSELEX'}</a>
</div>

<div class="z-admin-content-pagetitle">
    {icon type="view" size="small"}
    <h3>{gt text='ZSELEX Theme list'}</h3>
</div>

<form class="z-form" id="type_filter" action="{modurl modname='ZSELEX' type='admin' func='viewzselextheme'}" method="post" enctype="application/x-www-form-urlencoded">
    <fieldset id="zselex_multicategory_filter"{if $filter_active} class='filteractive'{/if}>
        ...
        <span class="z-nowrap z-buttons">
            ...
            <a href="{modurl modname="ZSELEX" type='admin' func='viewzselextheme'}" title="{gt text="Clear"}">{img modname=core src="button_cancel.png" set="icons/extrasmall" __alt="Clear" __title="Clear"} {gt text="Clear"}</a>
        </span>
    </fieldset>
</form>
       ...
       <table id="zselex_admintable" class="z-datatable">
            ...
            <tbody>
                ...
                <tr class="{cycle values='z-odd,z-even'}">
                    ...
                    <td>{$typeitem.zt_id|safetext}</td>
                    <td>{$typeitem.theme_name|safetext}</td>
                    <td>{$typeitem.description|safetext}</td>
               
                    <td>
                    <a href='{modurl modname='ZSELEX' type='admin' func='removeZselexTheme' id=$typeitem.zt_id}'>{gt text='Delete'}</a>
                    </td>
                </tr>
                {foreachelse}
