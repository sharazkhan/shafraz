        ...
        {button src="button_ok.png" set="icons/extrasmall" __alt="Save" __title="Save" __text="Save" __name="typetable[action]" __value="save"}
            ...
            <tbody>
                ...
                <tr class="{cycle values='z-odd,z-even'}">
                   ...
                   <td>
                    <a id="{$item.file_id}" rel="imageviewer[gallery]" title="{$item.imageName}" href="{$baseurl}modules/ZSELEX/images/shopgallery/{$item.imageName}">
                    <img src="{$baseurl}modules/ZSELEX/images/shopgallery/thumbs/{$item.imageName}"></a>
                    </td>
                
                    <td>{$item.shop_name|safetext} </td>
                    
                    <td><input type="radio" name="defaultimage" id="image{$item.gallery_id}" {if $item.defaultImg eq 1} checked="checked" {/if} value="{$item.gallery_id}" />  <label for="image{$item.gallery_id}">default</label></td>
