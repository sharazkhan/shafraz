<div class="news_index">
   ...
   {* <h3 class="news_title">{$preformat.title|safehtml}</h3> *}
    ...
    <h3 class="news_title"><a href="{modurl modname='ZSELEX' type='user' func='display' shop_id=$shop_id  sid=$info.sid}">{$info.title}</a></h3>
    ...
    <div class="news_body">
        ...
        <div class="news_photoindex news_thumbsindex" style="float:{$modvars.News.picupload_index_float}">
            {if $modvars.ZConfig.shorturls}
                ...
                <a href="{modurl modname='ZSELEX' type='user' func='display' shop_id=$shop_id sid=$info.sid from=$info.from urltitle=$info.urltitle}">{*<span></span>*}<img src="{$modvars.News.picupload_uploaddir}/pic_sid{$info.sid}-0-thumb.jpg" alt="{gt text='Picture %1$s for %2$s' tag1='0' tag2=$info.title}" /></a>
            {else}
                <a href="{modurl modname='ZSELEX' type='user' func='display' shop_id=$shop_id  sid=$info.sid}">{*<span></span>*}<img src="{$modvars.News.picupload_uploaddir}/pic_sid{$info.sid}-0-thumb.jpg" alt="{gt text='Picture %1$s for %2$s' tag1='0' tag2=$info.title}" /></a>
            {/if}
        </div>
        {/if}
    ...
    <p class="news_footer">
       ...
       {gt text='Filed under:'}
        ...
        <b>{$categorylink|strip_tags}</b>{if $smarty.foreach.categorylinks.last neq true},&nbsp;{/if}
        {/foreach}
