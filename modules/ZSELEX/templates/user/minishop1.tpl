...
<div class="OrageEditSec mainHeadEdit">
  ...
  <span align="right" class="right">
      <select name='prod_category'  id="prod_category" onchange="document.forms['cat_filter'].submit();">
          <option value=''>{if $prod_catId > 0}{gt text='All'}{else}{gt text='select category'}{/if}</option>
            {foreach from=$categories  item='item'}
                <option value="{$item.prd_cat_id}"  {if $item.prd_cat_id eq $prod_catId} selected="selected" {/if} > {$item.prd_cat_name} </option>
            {/foreach}
      </select>
  </span>
...
<form class="z-form" id="mnfr_filter"  action="{modurl modname="ZSELEX" type="user" func="shop" shop_id=$smarty.request.shop_id}" method="post" enctype="application/x-www-form-urlencoded">
  ...
  <span align="right" class="right">
      <select name='prod_mnfr'  id="prod_mnfr" onchange="document.forms['mnfr_filter'].submit();">
          <option value=''>{if $mnfr_id > 0}{gt text='All'}{else}{gt text='select manufacturer'}{/if}</option>
            {foreach from=$manufacturers  item='manufacturer'}
                <option value="{$manufacturer.manufacturer_id}"  {if $manufacturer.manufacturer_id eq $mnfr_id} selected="selected" {/if} > {$manufacturer.manufacturer_name} </option>
            {/foreach}
      </select>
  </span>
