...
<h3>{gt text='Create an article'} for the shop {$shopName}</h3>

<div class="z-admin-content-pagetitle">
     <h5><font color="red">{gt text='This Service Is Tempororily Disabled'}</font></h5>
</div>
<form id="news_user_newform" class="z-form" action="{modurl modname='News' type='user' func='create'}" method="post" enctype="{if $accesspicupload AND $modvars.News.picupload_enabled}multipart/form-data{else}application/x-www-form-urlencoded{/if}">
    <div>
        <input type="hidden" name="story[shop_id]" value="{$shop_id}" />
        <input type="hidden" name="story[shopName]" value="{$shopName}" />
        <input type="hidden" name="story[shop_idnewItem]" value="{$smarty.get.shop_idnewItem}" />
        ...
        <fieldset>
            ...
            <div class="z-formrow">
                ...
                <input  {if $admin < 1} disabled="disabled"  {/if} id="news_title" name="story[title]" type="text" size="32" maxlength="255" value="{$item.title|safetext}" />
            </div>

            {if $accessadd eq 1}
            <div class="z-formrow">
                <label for="news_urltitle">{gt text='Permalink URL'}</label>
                <input  {if $admin < 1} disabled="disabled"  {/if} id="news_urltitle" name="story[urltitle]" type="text" size="32" maxlength="255" value="{$item.urltitle|safetext}" />
        ...
        <fieldset class="z-linear">
            ...
            <div class="z-formrow">
                ...
                <textarea  {if $admin < 1} disabled="disabled"  {/if} id="news_hometext" name="story[hometext]" cols="40" rows="10">{$item.hometext|safetext}</textarea>
            ...
            <div class="z-formrow">
                ...
                <select  {if $admin < 1} disabled="disabled"  {/if} id="news_hometextcontenttype" name="story[hometextcontenttype]">
            ...
            <div class="z-formrow">
                ...
                <textarea  {if $admin < 1} disabled="disabled"  {/if} id="news_bodytext" name="story[bodytext]" cols="40" rows="10">{$item.bodytext|safetext}</textarea>
            ...
            <div class="z-formrow">
                ...
                <select  {if $admin < 1} disabled="disabled"  {/if} id="news_bodytextcontenttype" name="story[bodytextcontenttype]">
            ...
            <div class="z-formrow">
                ...
                <p id="news_notes_details">
                    <textarea  {if $admin < 1} disabled="disabled"  {/if} id="news_notes" name="story[notes]" cols="40" rows="10">{$item.notes|safetext}</textarea>
        ...
        <fieldset>
            ...
            <input  {if $admin < 1} disabled="disabled"  {/if} type="hidden" name="MAX_FILE_SIZE" value="{$modvars.News.picupload_maxfilesize|safetext}" />
            {if $modvars.News.picupload_maxpictures eq 1}
            <input  {if $admin < 1} disabled="disabled"  {/if} id="news_files_element" name="news_files[0]" type="file">
            {else}
            ...
            <div><br />
                ...
                <input  {if $admin < 1} disabled="disabled"  {/if} type="checkbox" id="story_del_picture_{$smarty.foreach.temp_pics.index}" name="story[del_pictures][]" value="{$file.name}">
            ...
            <div><br />
                ...
                <input   {if $admin < 1} disabled="disabled"  {/if} type="checkbox" id="story_del_picture_{$smarty.section.counter.index}" name="story[del_pictures][]" value="pic_sid{$item.sid}-{$smarty.section.counter.index}">
        ...
        <fieldset>
            ...
            <div id="news_publication_details">
                <div class="z-formrow">
                    ...
                    <input  {if $admin < 1} disabled="disabled"  {/if} id="news_displayonindex" name="story[displayonindex]" type="checkbox" value="1" {if $item.displayonindex eq 1}checked="checked" {/if}/>
                </div>
                <div class="z-formrow">
                    <label for="news_weight">{gt text='Article weight'}</label>
                    <div>
                        <input  {if $admin < 1} disabled="disabled"  {/if} id="news_weight" name="story[weight]" type="text" size="10" maxlength="10" value="{$item.weight|safetext}" />
                    </div>
                </div>
                <div class="z-formrow">
                    <label for="news_unlimited">{gt text='No time limit'}</label>
                    <input  {if $admin < 1} disabled="disabled"  {/if} id="news_unlimited" name="story[unlimited]" type="checkbox" value="1" {if $item.unlimited eq 1}checked="checked" {/if}/>
                </div>
                <div id="news_expiration_details">
                    <div class="z-formrow">
                        <label>{gt text='Start date'}</label>
                        <div>
                            <input  {if $admin < 1} disabled="disabled"  {/if} id="news_from" class="datepicker" name="story[from]" type="text" size="18" value="{$item.from}" />
                        </div>
                    </div>
                    <div class="z-formrow">
                        <label for="news_tonolimit">{gt text='No end date'}</label>
                        <input  {if $admin < 1} disabled="disabled"  {/if} id="news_tonolimit" name="story[tonolimit]" type="checkbox" value="1" {if $item.tonolimit eq 1}checked="checked" {/if}/>
                    </div>
                    <div id="news_expiration_date">
                        <div class="z-formrow">
                            <label>{gt text='End date'}</label>
                            <div>
                                <input  {if $admin < 1} disabled="disabled"  {/if} id="news_to" class="datepicker" name="story[to]" type="text" size="18" value="{$item.to}" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="z-formrow">
                    ...
                    <input  {if $admin < 1} disabled="disabled"  {/if} id="news_sid" readonly="readonly" name="story[sid]" size="5" value="{$item.sid}" />
                </div>
            </div>
        </fieldset>
                {/if}
