...
<form id="news_user_newform" class="z-form" action="{modurl modname='News' type='user' func='create' shop_id=$smarty.request.shop_id}" method="post" enctype="{if $accesspicupload AND $modvars.News.picupload_enabled}multipart/form-data{else}application/x-www-form-urlencoded{/if}">
    <div>
        <input type="hidden" name="story[shop_id]" value="{$shop_id}" />
        <input type="hidden" name="story[shopName]" value="{$shopName}" />
        <input type="hidden" name="story[purpose]" value="event" />
        <input type="hidden" name="story[modifyid]" value={$smarty.request.mid} />
        <input type="hidden" name="story[shop_idnewItem]" value="{$smarty.request.shop_idnewItem}" />
        ...
        <div class="z-buttonrow z-buttons z-center">
            ...
            <a id="news_button_cancel" href="{if $smarty.request.mid eq ''}{modurl modname='ZSELEX' type='admin' func='createevent' shop_id=$smarty.request.shop_id}{else}{modurl modname='ZSELEX' type='admin' func='modifyevent' shop_id=$smarty.request.shop_id eventId=$smarty.request.mid}{/if}" class="z-btred">{img modname='core' src='button_cancel.png' set='icons/extrasmall' __alt='Cancel' __title='Cancel'} {gt text='Cancel'}</a>
