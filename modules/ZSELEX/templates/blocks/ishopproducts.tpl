...
<div id="blockcontent"  style="width:380px; height:auto; display:table-cell;">
    <dl>
        ...
        <div style="float:left; width:170px; margin-left:15px; display:table-cell;">
         ...
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.product_id}"><img src="{$baseurl}zselexdata/{$item.uname}/products/thumb/{$item.prd_image}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {else}
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.product_id}"><img src="{$baseurl}zselexdata/{$item.uname}/products/thumb/{$item.prd_image}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {/if}
            </dt>
