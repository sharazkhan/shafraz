 {assign var=functionss value=','|explode:'findus,site,minishop,productview,site'}
...
<div id="navcontainer_{$blockinfo.bid}" class="navcontainer">
    {menu from=$menuitems item='item' name='extmenu' class='navlist'}
     {if $item.name != '' && $item.url != ''}
      ...
      {* <li{if $smarty.request.func eq $item.func || $item.type eq 'admin'} class="selected" {/if}>*}
     
        {*<li{if $smarty.request.func eq $item.func} class="selected" {/if}>*}
