...
function smarty_function_cartcount2($args, &$smarty)
{

    // echo "<pre>"; print_r($_SESSION['cart']); echo "</pre>"; exit;
    // $item = ZSELEX_Controller_User::carttotal();
    // exit;
    $item             = ModUtil::apiFunc('ZSELEX', 'cart', 'carttotal',
            $curr_args);
    ...
    $GRANDTOTAL_FINAL = ModUtil::apiFunc('ZSELEX', 'user', 'number2currency',
            $curr_args);
    $counts           = "&nbsp;";
    if (!empty($item ['count'])) {
        $counts = $item ['count'];
   ...
   // return $GRANDTOTAL_FINAL . ' DKK';
    $smarty->assign ( "cartCount", $GRANDTOTAL_FINAL );
    $smarty->assign ( "cartTotal", $item ['total'] );
