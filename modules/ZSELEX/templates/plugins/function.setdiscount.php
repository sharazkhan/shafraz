...
function smarty_function_setdiscount($args, &$smarty) {
	...
	$discount = 0;
	if (! empty ( $args ['value'] ) || $args ['value'] > 0) {
		$discount = $args ['value'];
	} else {
		$em = ServiceUtil::getService ( 'doctrine.entitymanager' );
		$repo = $em->getRepository ( 'ZSELEX_Entity_Product' );
		$discount = $repo->getZeroQtyDiscount ( array (
	...
	if (! empty ( $discount ) && $discount > 0) {
		...
		if ($last_char == "%") {
			...
			// alert(newVal);
			$discount_price = $origPrice - ($origPrice * $newVal / 100);
			...
			$discount_price = $origPrice - $discount;
	...
	$smarty->assign ( "dicount_price", $discount_price );
}
