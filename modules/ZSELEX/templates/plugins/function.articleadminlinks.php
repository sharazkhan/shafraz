...
function smarty_function_articleadminlinks($params, &$smarty) {
	...
	if (SecurityUtil::checkPermission ( 'News::', "$info[cr_uid]:$info[cattitle]:$info[sid]", ACCESS_READ ) /* || ($uid != 0 && $info['cr_uid'] == $uid) */) {
		...
		if (isset ( $params ['type'] ) && $params ['type'] == 'ajax') {
			...
			smarty_function_ajaxheader ( array (
					'modname' => 'News',
					'filename' => 'news.js' 
			), $smarty );
			smarty_function_ajaxheader ( array (
					'modname' => 'News',
					'filename' => 'sizecheck.js' 
			), $smarty );
			smarty_function_ajaxheader ( array (
					'modname' => 'News',
					'filename' => 'prototype-base-extensions.js' 
			), $smarty );
			smarty_function_ajaxheader ( array (
					'modname' => 'News',
					'filename' => 'prototype-date-extensions.js' 
			), $smarty );
			smarty_function_ajaxheader ( array (
					'modname' => 'News',
			...
			if (SecurityUtil::checkPermission ( 'News::', '::', ACCESS_ADMIN )) {
				$articlelinks .= '<span class="' . $params ['class'] . '"> ' . $params ['start'] . ' <a href="' . DataUtil::formatForDisplayHTML ( ModUtil::url ( 'News', 'admin', 'modify', array (
						'sid' => $params ['sid'] 
				) ) ) . '">' . __ ( 'Edit', $dom ) . '</a>';
			}
			...
			if (SecurityUtil::checkPermission ( 'News::', '::', ACCESS_ADMIN )) {
				$articlelinks .= ' ' . $params ['seperator'] . ' <a href="' . DataUtil::formatForDisplay ( ModUtil::url ( 'News', 'admin', 'delete', array (
						'sid' => $params ['sid'] 
				) ) ) . '">' . __ ( 'Delete', $dom ) . '</a>';
			}
