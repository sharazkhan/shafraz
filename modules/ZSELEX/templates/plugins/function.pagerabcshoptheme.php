 ...
 *        	Reference to the Zikula_View object.
 *        	
 * @return string
 */
function smarty_function_pagerabcshoptheme($params, Zikula_View $view) {
	...
	if (! empty ( $params ['names'] )) {
		...
		if (strtolower ( $params ['skin'] ) == 'hu') {
			// Hungarian
			$pager ['names'] = $pager ['values'] = array (
					'A',
					'?',
					'B',
					'C',
	...
	foreach ( array_keys ( $pager ['names'] ) as $i ) {
		...
		// $pager['args']['shop_id'] = $pager['shop_id'];
		$urltemp = DataUtil::formatForDisplay ( ModUtil::url ( $pager ['module'], $pager ['type'], $pager ['func'], $pager ['args'] ) );
		$urltemp = $urltemp . "&shop_id=$params[shop_id]";
		if ($i > 0) {
