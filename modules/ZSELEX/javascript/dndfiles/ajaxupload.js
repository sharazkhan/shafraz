 ...
 * 
 */
//Zikula.define('ZSELEX');
(function($)
{
    // alert(Zikula.Config.lang);
    //alert(Zikula.__('Add Files'));
    // alert(Zikula.__('Add Files' , 'module_zselex_js'));
    ...
    function getURL(settings, fileName, size)
    {
        ...
        params.push('shop_id=' + shop_id);
        //alert(settings.data);
        params.push('data=' + settings.data);
    ...
    {
        // var servicelimit  =  document.getElementById('servicelimit').value;
        // settings.maxFiles = servicelimit;
        // jQuery(".aj-frame").css("display","block");
        var FILES = settings.FILES;
    ...
    function clearQueue(settings)
    {
        ...
        while(settings.FILES.length>0)
            // alert(settings.FILES[0]);
            settings.FILES[0].remove();
    }
    ...
    var globalSettings = 
    {
        ...
        error:			function(txt,fileName){
            alert('Problems with the upload :' + txt);
            redirect();
        ...
        maxFileSize:	'2MB',					//max file size of single file,
    ...
    var methods =
    {
        init : function(options)
        {
            ...
            {	
                ...
                var fieldSet  	= $('<fieldset class="aj-frame" />').append('<legend class="ax-legend">'+Zikula.__("Select files",'module_zselex_js')+'</legend>').appendTo($this);
                var browse_c  	= $('<a class="ax-browse-c ax-button" title="'+Zikula.__('Add Files' , 'module_zselex_js')+'" />').appendTo(fieldSet);//Browse container
                browse_c.append('<span class="ax-plus-icon ax-icon"></span> <span>'+Zikula.__('Add Files' , 'module_zselex_js')+'</span>');
				
                var browseFiles = $('<input type="file" class="ax-browse" name="ax-files[]" />').attr('multiple',isAjaxUpload).appendTo(browse_c);
				
                var uploadFiles = $('<a class="ax-upload-all ax-button" title="'+Zikula.__("Upload all files" , 'module_zselex_js')+'" />').appendTo(fieldSet);
                uploadFiles.append('<span class="ax-upload-icon ax-icon"></span> <span>'+Zikula.__("Start upload" , 'module_zselex_js')+'</span>');
			    
                var removeFiles = $('<a class="ax-clear ax-button" title="'+Zikula.__("Clear all" , 'module_zselex_js')+'" />').appendTo(fieldSet);
                removeFiles.append('<span class="ax-clear-icon ax-icon"></span> <span>'+Zikula.__("Remove all" , 'module_zselex_js')+'</span>');
