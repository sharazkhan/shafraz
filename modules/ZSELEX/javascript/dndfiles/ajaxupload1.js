...
(function($)
{
    ...
    function getURL(settings, fileName, size)
    {
        ...
        //alert(settings.url+c+params.join('&')); exit();
    ...
    function uploadAjax(queued, settings)
    {
        ...
        // var randnum = getRandomInt(1, 10000000);
        // alert(randnum+"_"+name); exit();
        // name = randnum+"_"+name;
        ...
        xhr.onreadystatechange=function()
        {
            if(this.readyState == 4 && this.status == 200)
            {
                try
                {
                    ...
                    //alert(ret.servicecount); exit();
//                    alert(ret.status); exit();
                    document.getElementById('servicelimit').value = ret.servicecount;
                    
                    if(ret.status == 'error')
                    {
                        throw ret.info;
                    }
                    if(ret.servicecount<1){
                        //alert(ret.msg); exit();
                        document.getElementById('limover').innerHTML = "<b><font color='red'>"+ret.msg+"</font></b>";
                   
                    //return false;
                    //return;
                       
                    }
                    else if(ret.diskquota=='-1'){
                        document.getElementById('limover').innerHTML = "<b><font color='red'>"+ret.msg+"</font></b>";
                    //return;
                    }
                    if(startByte == 0)
                    ...
                    {
                        queued.byte = endByte;
                        uploadAjax(queued, settings); // removed by me for error
                    //settings.SLOTS--;
                    //queued.end(ret.name, ret.size, ret.status, ret.info);
                    }
                }
    ...
    function ajaxList(list, obj, settings)
    {
        ...
        // obj.remove(); // remove each files after upload
        };
        ...
        {
            for (var t = 0; t < dataArray.length; t++) { // avoid additional upload of same file
                if(dataArray[t].fnamez==this.name){
                    //return;
                }
            }
            if(this.xhr !== null)
            ...
            else if(settings.SLOTS < settings.maxConnections)
            {
                ...
                this.tools.pNum.html('Uploading...');
                this.tools.pBar.css('width','0%');
                this.tools.upload.addClass('ax-abort');
                // alert(this.name); exit();
                //var randnum = getRandomInt(1, 10000000);
                // var tim = new Date().getTime();
                //alert(this.name); exit();
                // this.name= this.name.replace(/\s+/g, '');
                // alert(name_pure); exit();
                // this.name = tim+"_"+this.name;
    ...
    function doneAll(){
        //jQuery('#plswait').attr("align=center")
        // document.getElementById('plswait').innerHTML = '<h2>'+Zikula.__('Your changes wil be reloaded after few seconds')+'</h2>';
        setTimeout(function() {
    ...
    var globalSettings = 
    {
        ...
        thumbHeight:	150,							//max thumbnial height if set generate thumbnial of images on server side
        ...
        thumbPath:		'thumb',							//set the path where thumbs should be saved, if empty path setted as remotePath
        thumbFormat:	'jpg',							//default same as image, set thumb output format, jpg, png, gif
        maxFileSize:	'10001M',					//max file size of single file,
