...
;(function(jQuery) {
    //alert('hi');
    jQuery.fn.ZselexCombo = function(config) {
    ...
    var defaults = {
	   ...
	   //Also applied when user types something that is not in the options list
	    ...
	    //all callback functions are called in the scope of the current ZselexCombo instance
    ...
    jQuery.ZselexCombo = function(selectbox, config) {
	    ...
	    if ((this.config.checkWidth) && (this.listWrapper.innerWidth() < maxOptionWidth)) {
		    //this.overflowCSS = "overflow"; // oroginal
                    this.overflowCSS = "overflow-y"; // modified
    ...
    //shortcuts
    var $sc = jQuery.ZselexCombo;
    $sc.fn = $sc.prototype = {};
    $sc.fn.extend = $sc.extend = jQuery.extend;
    ...
    createSelectbox: function(config) {
	    ...
	    for (var i = 0, len = data.length; i < len; ++i) {
	        ...
	        jQuery("<option />").appendTo($selectbox).
