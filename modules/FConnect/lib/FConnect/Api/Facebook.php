...
class FConnect_Api_Facebook extends Zikula_AbstractApi
{
    ...
    public function logInUrl($args = null)
    {
        if ($args['gobackurl'] == null) {
            $args['gobackurl'] = ModUtil::url($this->name, 'user', 'main',
                    $args              = array(), $ssl               = null,
                    $fragment          = null, $fqurl             = true,
                    $forcelongurl      = false, $forcelang         = false);
        }

        if ($args['scope'] == null) {
            //$args['scope'] = 'email,read_stream';
            $args['scope'] = 'email,public_profile,user_friends';
        }

        return $this->_facebook->getLoginUrl($params = array('scope' => $args['scope'],
                'redirect_uri' => $args['gobackurl']));
    }
}