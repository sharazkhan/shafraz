...
$redirecturl = ModUtil::url('ZSELEX', 'user', 'error');
$articleAllowed = ModUtil::apiFunc('ZSELEX', 'user', 'checkArticlePermission', $args=array('user'=>$user));
//echo "Article Count: ". $articleAllowed;
...
$perm = ModUtil::apiFunc('ZSELEX', 'user', 'permission', $args=array('user'=>$user));
//echo $perm ;

$ishops = ModUtil::apiFunc('ZSELEX', 'user', 'getOwnerShops', $args=array('user'=>$user));

//echo "<pre>"; print_r($ishops); echo "</pre>";

{/php}

{assign var="vals" value=""}
...
<form id="news_user_newform" class="z-form" action="{modurl modname='News' type='user' func='create'}" method="post" enctype="{if $accesspicupload AND $modvars.News.picupload_enabled}multipart/form-data{else}application/x-www-form-urlencoded{/if}">
    <div>
        ...
        <fieldset>
            ...
            echo "<div class='z-formrow'>";
                echo "<label>Shop</label>";
                echo "<select name='SHOP'>";
                    foreach($ishops as $shop){
                    echo "<option value='".$shop[shop_id]."'>$shop[shopname]</option>";
                       }
                    echo "</select>";
