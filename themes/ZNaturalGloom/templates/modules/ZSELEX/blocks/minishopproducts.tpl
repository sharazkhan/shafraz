...
<div id="blockcontent"  style="width:auto; height:auto;">
    <dl>
        {foreach item='item' from=$products}
         ...
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.productId}"><img class="lazy"  src="{$baseurl}images/grey_small.gif" data-original="{$baseurl}modules/ZSELEX/images/products/thumbs/{$item.prdImage}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {else}
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.productId}"><img class="lazy"  src="{$baseurl}images/grey_small.gif" data-original="{$baseurl}modules/ZSELEX/images/products/thumbs/{$item.prdImage}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {/if}
            </dt>
            ...
            <div class="P1TBox">
                ...
                <div class="add">
                    ...
                    <form name="cart_quantity{$item.productId}" action="{modurl  modname='ZSELEX' type='user' func='cart'}" {*action="{$baseurl}index.php?module=zselex&type=user&func=cart"*} method="post" enctype="multipart/form-data" target="_blank">
                        <input type="hidden" name="cart_quantity" value="1" />
                        <input type="hidden" name="productId" value="{$item.productId}" /> 
                        <input type="hidden" name="productName" value="{$item.prdName}">
                        <input type='hidden' name='product_price' value="{$item.prdPrice}" >
                        <input type='hidden' name='productImage' value="{$item.prdImage}" > 
                        <input type='hidden' name='productDesc' value="{$item.prdDescription}" >
                        <input type='hidden' name='shopId' value="{$item.shopId}" ></form>
                </div>
            </div>
        </div>
