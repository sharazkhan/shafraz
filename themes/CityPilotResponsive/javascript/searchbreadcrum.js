...
function searchBreadcrums() {
    ...
    if ((jQuery('#hregion').val() > 0 || getCookie('region_cookie') > 0)) {
        ...
        jQuery('.BrudcomeTree').append('<li id=bc_reg><a href="#">' + getRegionCookie + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_reg")></i></li>');
    }

    if ((jQuery('#hcity').val() > 0 || getCookie('city_cookie') > 0)) {
        jQuery("#bc_city").remove();
        jQuery('.BrudcomeTree').append('<li id=bc_city><a href="#">' + getCityCookie + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_city")></i></li>');
    }
    ...
    if ((jQuery('#harea').val() > 0 || getCookie('area_cookie') > 0)) {
        ...
        jQuery('.BrudcomeTree').append('<li id=bc_area><a href="#">' + getAreaCookie + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_area")></i></li>');
    }

    if ((jQuery('#hcategory').val() > 0 || getCookie('category_cookie') > 0)) {
        jQuery("#bc_cat").remove();
        jQuery('.BrudcomeTree').append('<li id=bc_cat><a href="#">' + getCategoryCookie + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_cat")></i></li>');
    }
    ...
    if ((jQuery('#hsearch').val() != '' && jQuery('#hsearch').val() != Zikula.__('search for...', 'module_zselex_js') || getCookie('search_cookie').len > 0)) {
        ...
        jQuery('.BrudcomeTree').append('<li id=bc_search><a href="#">' + escape(stripslashes(getSearchCookie)) + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_search")></i></li>');
    }
    ...
    if (getAffiliateCookie.length > 0) { //aff
        ...
        if (lock == 1) {
            ...
            jQuery('.BrudcomeTree').append('<li id=bc_affiliate><a href="#">' + getAffiliateCookie + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_affiliate")></i></li>');
        }
    }
