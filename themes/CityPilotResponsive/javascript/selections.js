...
function regionSelect(regionId, regionName) {   // called on region on change
    ...
    if (regionId > 0) {
        jQuery('.BrudcomeTree').append('<li id=bc_reg><a href="#">' + regionName + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_reg")></i></li>');
    }
...
function citySelect(cityId, cityName) {
    ...
    jQuery("#bc_area").remove();
    if (cityId > 0) {
        jQuery('.BrudcomeTree').append('<li id=bc_city><a href="#">' + cityName + '</a><i class="fa fa-times remove-icon" onclick=closeBcLi("bc_city")></i></li>');
    }
    // }
