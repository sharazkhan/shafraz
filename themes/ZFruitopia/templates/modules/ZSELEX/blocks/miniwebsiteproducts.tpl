...
<div id="blockcontent"  style="width:400px; height:auto; display:table-cell;">
    <dl>
        ...
        <div style="float:left; width:170px; margin-left:15px; display:table-cell;">
         ...
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.productId}"><img src="{$baseurl}modules/ZSELEX/images/{$ownerName}/products/thumb/{$item.prdImage}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {else}
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.productId}"><img src="{$baseurl}modules/ZSELEX/images/{$ownerName}/products/thumb/{$item.prdImage}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {/if}
            </dt>
