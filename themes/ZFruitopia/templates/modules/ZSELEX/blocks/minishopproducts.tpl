...
<div id="blockcontent"  style="width:auto; height:auto;">
    <dl>
        {foreach item='item' from=$products}
         ...
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.productId}"><img class="lazy"  src="{$baseurl}images/grey_small.gif" data-original="{$baseurl}modules/ZSELEX/images/{$ownerName}/products/thumb/{$item.prdImage}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {else}
         <a target="_blank" href="{modurl  modname='ZSELEX' type='user' func='productview' id=$item.productId}"><img class="lazy"  src="{$baseurl}images/grey_small.gif" data-original="{$baseurl}modules/ZSELEX/images/{$ownerName}/products/thumb/{$item.prdImage}"  {if $item.H  neq  ''} height='{$item.H}'  width='{$item.W}' {else} width='170px'  {/if}/></a>
           {/if}
            </dt>
