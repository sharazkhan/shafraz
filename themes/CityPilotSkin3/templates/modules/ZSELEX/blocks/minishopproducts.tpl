    ...
    <ul class="Product-Images-Sequence FullProduct">
        ...
        <li> 
            <div class="FlexiImg">
              ...
              {if $is_discount}<div class="Circle"> <p class="CText">{$dicount_value}</p></div>{/if}
             <p class="PText tooltips" title="{$item.product_name}"><span>{shorttext text=$item.product_name len=10}</span><span class="right">{displayprice amount=$item.prd_price} DKK</span>
            </p>
...
<div>
    <ul class="Product-Images-Sequence">
        ...
        <li> 
            ...
            {* <div class="Box" onClick="document.cart_quantity{$item.products_id}.submit()">{gt text='Køb'}</div>*}
            <div class="Box"><a href="javascript:document.cart_quantity{$item.products_id}.submit()"  class="addbutton">{gt text='Køb'}</a></div>
